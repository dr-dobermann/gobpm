...
- [Persistence](docs/guides/persistence.md) — instance checkpoints & restart recovery: arming with `WithRepository`, what's written and when, per-wait recovery semantics (overdue timers fire once), ownership leases + CAS fencing for shared stores, stable element ids as the deployment-parity contract
