...
func main() {
    ...
    // Add service task
    serviceTask, err := activities.NewServiceTask("process-data")
    if err != nil {
    ...
    // In real usage, you'd use sequence flows
    ...
    // Execute process
    snapshot, err := engine.StartProcess(ctx, "simple-process", nil)
    if err != nil {
        log.Fatal("Failed to start process:", err)
    }
    
    log.Printf("Process started with ID: %s", snapshot.Id())
}
```
...
```go
// Create timer event that triggers after 30 seconds
timerDef, err := events.NewTimerEventDefinition(
    nil, // timeCycle
    nil, // timeDuration  
    data.MustFormalExpression(goexpr.MustNew("30s")), // timeDate
)

timerEvent, err := events.NewIntermediateEvent("timer-30s", timerDef)
```
