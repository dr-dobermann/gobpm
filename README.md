...
`make ci` is the contract: green locally ⇒ green on CI. The Go toolchain is pinned (`go.mod` → `go1.25.13`) so local and CI scan the identical standard library.
...
- Go (toolchain pinned to `go1.25.13` via `go.mod`; `GOTOOLCHAIN=auto` fetches it automatically)
...
- [Interchange converters](docs/guides/extending/converters.md) — import and export BPMN 2.0 XML: the format-agnostic `convert` seam, blank-import registration, id preservation as the version key, unsupported-element feedback, semantic round-trip, reuse by reference (a `<globalTask>` imports as a callable process; a namespace-qualified `calledElement` is mapped by the host's `WithCallableResolver`)
