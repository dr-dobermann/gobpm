...
| **`GlobalTask` as a `CallActivity` target** (§10 — "the elements … called by Call Activities … are: Process and GlobalTask") | **the `gobpm-server` product**, once it carries a task registry | the library covers the same need by construction (below) |

**Why `GlobalTask` sits on the server side.** A `GlobalTask` is reuse **by reference**: one definition at `Definitions` scope, called by many `CallActivity`s. That requires a registry of callable definitions, and a registry is a product concern — the same reasoning that puts import there. The element exists in BPMN because **XML has no functions**: a file cannot call a builder, so the standard needs a named, referenceable definition to get reuse at all. The library has functions, so it covers the authoring need by construction — a Go constructor returning a configured task *is* a reusable definition, and a better one, since it can be parameterized. What the library does **not** offer is by-reference reuse; today the only by-reference path is `CallActivity` against the process registry, which launches a child instance. When the server's task registry lands, `CallActivity` resolves a key to a task and `GlobalTask` becomes accessible with no model change. Until then the converter must **refuse** a `<globalTask>` loudly rather than drop it: unlike `Lane`, a GlobalTask is *callable*, so carrying it inertly would import a model that fails at run time.
