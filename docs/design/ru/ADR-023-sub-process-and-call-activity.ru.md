...
| Статус | Принято |
| Версия | v.4 |
| Дата | 2026-07-16 (v.1 принято 2026-07-17; v.2 принято 2026-07-18; v.3 черновик 2026-07-22, принято 2026-08-08; v.4 2026-08-26) |
...
> **Принято (v.3)** — поверх принятой v.2: §2.3 закрепляет заметку о
> `startQuantity`/`completionQuantity` как осознанный non-goal
> (→ SAD-001 §4 N8), а §2.7 получает **контракт рестарта** — вызов в
> полёте переживает рестарт движка целиком (долговечный ребёнок,
> записанный вызов, восстановление пересвязывает оба конца; управляется
> ADR-033 v.4 §2.10); концепция модели выполнения в остальном не
> изменена. v.2 расширила принятую v.1 решением об **Event Sub-Process**
> (§2.10), продвигая его из v.1-шва «спроектировано под» (§2.8) к полноценной
> концепции; прерывающий слайс приземлён (сопровождающий SRD). Краеугольный
> камень v.1 остаётся принятым: **встроенный Sub-Process** (§2.2–§2.6) как
> **вложенная область выполнения внутри того же экземпляра** — один loop, один
> единственный писатель, *дерево* областей там, где сегодня одна плоская
> область — и **Call Activity** (§2.7) как **дочерний экземпляр** отдельно
> зарегистрированного процесса, вызываемый через версионированный реестр со
> стандартным прямым отображением I/O (эпик #85, приземлён). Всё это несёт одна
> концепция — **область** (scope): совместный контекст токенов/данных/событий из
> §10.5.7 — открывается при входе в композитную activity, завершается, когда **в
> ней не остаётся токенов** (§13.3.4), и отменяема **как единое целое**
> (кооперативная отмена ADR-018, применённая к композитному хосту). v.2
> реализует следующего наездника на этой области: **Event Sub-Process** (§2.10,
> эпик #91) — это `triggeredByEvent`-обработчик, **взведённый пока открыта его
> охватывающая область** — паттерн boundary-watch, поднятый с окна activity на
> окно области — с бюджетом прерывающего обработчика, **разделяемым с граничными
> событиями**, и правилом precedence **absorb-vs-re-throw**. Граничные события на
> композитах, **scoped** Terminate End Event (§13.5.6) и обход **цепочки
> областей** для Error, который ADR-006 §2.6 смоделировал, а ADR-018 отложил, —
> всё живо с v.1. По-прежнему спроектировано под, но ещё не решено: Transaction,
> Ad-Hoc, multi-instance (§2.8). Реализация нарезана сопровождающими SRD;
> краеугольный камень области — эпик #85, event sub-process — эпик #91.
...
**осознанный non-goal** (см. [SAD-001 v.1](../SAD-001-vision-and-architecture.md)
§4 N8): неявное размножение/слияние токенов без нотации на диаграмме делает
поведение непрозрачным, Camunda их не поддерживает, а явный Parallel
Gateway покрывает намерение видимо. Runtime соблюдает дефолтную 1.
...
- **I/O — прямое отображение из стандарта** (§10.4/семантика данных,
  ...
  должен исполняться идентично, как бы до него ни дошли). Сторона вызываемого
  в этом отображении — собственные объявленные входы и выходы процесса, что
  привязывается при его запуске и что читается при его завершении, и проверка
  параметров вызывающего по именам против них при запуске — это
  [ADR-040 v.1](../ADR-040-process-io-contract.md).
...
- **Контракт рестарта — вызов переживает рестарт целиком** (стандарт
  молчит о рестартах движка; выбор движка, управляемый правилом
  composite-fidelity из Persistence & State ADR, ADR-033 v.4 §2.10):
  дочерний экземпляр **долговечен сам по себе** — он чекпоинтится в том
  же репозитории и по той же дисциплине, что любой экземпляр, неся
  связку с родителем; чекпоинт вызывающего записывает вызов в полёте
  (id ожидаемого дочернего экземпляра + узел вызова). Восстановление
  поднимает **оба конца и пересвязывает их**: припаркованный трек
  вызывающего возобновляет ожидание восстановленного ребёнка,
  терминальное состояние ребёнка входит в восстановленного вызывающего
  ровно так же, как в резидентного, и каскад отмены выше переживает
  рестарт. Восстановленный вызывающий, чья запись ожидаемого ребёнка
  отсутствует, **громко проваливает своё восстановление** — вызов есть
  записанное состояние, поэтому тихий перезапуск ребёнка
  (дублированный дочерний экземпляр) исключён конструкцией; и наоборот,
  восстановленный ребёнок, чья родительская запись исчезла, падает
  громко, а не работает сиротой.

### 2.8 Спроектировано под: что поедет на модели области дальше

Решено здесь как **швы**, реализуется собственными workstream'ами:

- **Event sub-processes** (#91): **теперь решено в §2.10 (v.2)** — контейнер
  обработчиков, взведённый пока открыта его охватывающая область, продвинутый из
  этого шва к полноценной концепции. Шов был верен; §2.10 фиксирует бюджет,
  precedence и жизненный цикл, которые он отложил.
- **Transaction** (#91) — **реализовано** ([ADR-028](../ADR-028-transaction-sub-process.md)):
  вариант sub-process, чей Cancel End / Cancel boundary едут на scope-cancel плюс
  компенсации — ничто в этой модели этому не мешало.
...
### 2.10 Event Sub-Process — обработчик, взведённый областью (v.2)

**Event Sub-Process** (BPMN §13.5.4) — это `SubProcess`, помеченный
`triggeredByEvent`: фрагмент-обработчик, который живёт **внутри** области и
**взводится событием**, а не достигается sequence flow. Это scope-уровневый
аналог граничного события — где boundary охраняет окно одной activity, event
sub-process охраняет окно целой **области**. Это реализует шов §2.8 с бюджетом,
precedence и жизненным циклом, которые тот шов отложил.

**Модель.** Event sub-process переиспользует container области §2.2 (тот же
внутренний граф, тот же вложенный жизненный цикл/данные области §2.4) и добавляет
ровно то, что стандарт требует и запрещает:

- **Единственный триггерный Start Event** (§10.5.2 p.241: *«An Event Sub-Process
  MUST have a single Start Event»*) одного вида — Message, Timer, Signal, Error
  или Conditional в этом workstream'е (Escalation едет с #90); флаг
  `isInterrupting` старта выбирает вариант.
- **Самодостаточен** (§13.5.4): нет sequence flow к графу родителя или из него —
  достигается *только* срабатыванием своего старта. Поскольку его старт
  *триггерный*, он **никогда** не является узлом входа: инстанциация §2.3 сидит
  None-start / flow-less узлы и **пропускает** event sub-process, регистрируя его
  вместо этого как **обработчик области**.
- **Нет граничных событий** на нём (§13.5.4).
- **Работает в контексте данных родителя** (§13.5.4): его внутренние узлы читают
  охватывающую область через обход вверх §2.4/§10.5.7, как любой внутренний узел.

**Взведение — паттерн boundary-watch на гранулярности области.** Когда область
открывается (§2.4, или корень экземпляра при старте), loop **взводит** каждый
event sub-process, объявленный непосредственно в ней, регистрируя триггер старта
той же per-kind-машинерией, что использует граничное событие:

| Триггер | Механизм взведения |
|---|---|
| Message | hub-waiter по ключу сообщения, доставляемый как событие loop'а |
| Signal | hub-waiter на имя сигнала (broadcast-safe) |
| Timer | подсистема таймеров, срабатывающая в loop |
| Conditional | **loop-local conditional subscription** (ADR-006 v.3 §2.7) — фронт false→true; это то, куда приземляется **conditional start**, который ADR-006 v.3 отложил (conditional start легально читает охватывающую область, §10.4.3) |
| Error | **обход цепочки областей** §2.6 в точке throw — взведённого waiter'а нет; обход находит внутреннейшую охватывающую область, чей обработчик ловит код |

Обработчик взведён на **время жизни своей области** — разоружается, когда область
дренируется (§2.4), отменяется (§2.5) или, для прерывающего обработчика, как
только **любой** прерывающий обработчик в области сработал (бюджет ниже).

**Прерывающий — отмени область, запусти обработчик в ней.** На прерывающем
триггере (`isInterrupting=true`):

1. Loop **отменяет сиблинг-треки охватывающей области** — `cancelScope` из §2.5,
   применённый к *собственной* области обработчика. Плоскость данных области
   **остаётся открытой**; обработчик работает в ней.
2. Жизненный цикл родителя отражает триггер (§13.5.4): **Error**-старт переводит
   родителя в *Failing*, **не-error** прерывающий старт — в *Terminating*. У gobpm
   **нет отдельного состояния токена Failing/Terminating** (согласуется с выбором
   §2.1 не добавлять состояния жизненного цикла activity, которых loop'у не надо);
   различие реализуется тем, *какой* путь отмены выполняется (обход Error §2.6 vs
   простая scope-отмена), и выносится в наблюдаемость.
3. Loop **сидит внутренний поток обработчика** от его триггерного старта (старт
   трактуется как сработавший, его payload связан born-event-способом §2.4) в
   очищенную область; обработчик работает до своего End в контексте данных области.
4. Когда обработчик **дренируется**, область завершается и возобновляет хост-токен
   родителя (drain-resume §2.4), следуя исходу обработчика.

**Разделяемый бюджет прерывающего и absorb-vs-re-throw.** Для данного Event
Declaration **только ОДИН прерывающий обработчик** может сработать в области — *и
этот бюджет разделяется между event sub-process и любым граничным событием на том
же хосте области* (§10.5.6 p.278). Как только прерывающий обработчик стартовал,
**никакой дальнейший обработчик** не может быть инициирован для этой области.
gobpm обеспечивает это **одним реестром прерывающего взведения на область**, с
ключом по Event Declaration, в который пишут и boundary-watch, и взведение
event-sub'а; первое прерывающее срабатывание переводит область в *interrupted* и
разоружает остальные. Когда у области есть и инлайн event sub-process, **и**
boundary для того же `EventDefinition` (§10.5.6 / p.278 precedence):

- обработчик **завершается без re-throw** → он **поглощает** событие: boundary
  **не** срабатывает, родитель возобновляется на своём нормальном потоке;
- обработчик **пере-бросает** событие (Throw того же вида на своём End) → boundary
  срабатывает **после** обработчика: инлайн-обработчик — это **декоратор**, а не
  терминал.

Это даёт моделирующему явный контроль терминал-vs-декоратор; путь **absorb** —
первичный в этом workstream'е, re-throw компонуется с существующей машинерией
throw + boundary.

**Непрерывающий (решён; реализация нарезана вторым слайсом).** Непрерывающий
event sub-process (`isInterrupting=false`, любой триггер кроме Error — ошибки
всегда прерывают, §10.5.6) работает **конкурентно** с родителем: триггер
поглощается, экземпляр обработчика спавнится как **новый трек в свежей дочерней
области** под родителем, и родитель продолжает работать. Несколько могут работать
одновременно (§10.5.6: без ограничения, недетерминированный порядок); каждый —
своя область, и родитель не drain-completes, пока каждый заспавненный обработчик
не дренируется (drain-учёт области §2.4 уже считает каждый трек под путём). Ничто
в прерывающем дизайне этого не исключает — непрерывающее срабатывание — это
«заспавни трек обработчика, не отменяй область». **Реализация нарезана вторым
слайсом** (сопровождающие SRD приземляют прерывающий первым).
...
| Event sub-process (§2.10): `triggeredByEvent`; единственный триггерный start; самодостаточен; нет граничных событий; контекст данных родителя; Error→Failing, не-error прерывающий→Terminating; Completing ждёт активных обработчиков | §13.5.4 p.436-439; §10.5.2 p.241 («An Event Sub-Process MUST have a single Start Event») |
| ОДИН прерывающий обработчик на Event Declaration, **разделяемый** event-sub + boundary; непрерывающих без ограничения; наборы триггеров прерывающих/непрерывающих (Error только прерывающий) | §10.5.6 p.278 (`event-handling.md` §5) |
| Инлайн-обработчик **absorb** (нет re-throw → boundary подавлен) vs **re-throw** (→ boundary срабатывает после) | §10.5.6 p.278 (`event-handling.md` §6) |
| Conditional-старт легально читает охватывающую область (потому и приземляется с event sub-processes, а не на верхнем уровне) | §10.4.3 |
...
состояния жизненного цикла родителя Failing/Terminating, реализованные путём
отмены + наблюдаемостью, а не отдельным состоянием токена (§2.10) — никаких
состояний жизненного цикла activity, которых loop'у не надо; конкурентные
непрерывающие обработчики работают в недетерминированном порядке треков loop'а
(§10.5.6, соблюдено отсутствием гарантии порядка).
...
| **Event sub-processes как top-level-реестр обработчиков** (глобальные catcher'ы, §2.10) | Ломает scope-локальность, которую предписывает стандарт (обработчик ловит только внутри своей области), и дублирует цепочку областей §2.6. Event sub-process *и есть* область (контекст данных, drain, cancel) — scope-параллельный рантайм заново реализовал бы §2.2–§2.5. |
| **Моделировать event sub-processes как N граничных событий на хосте области** (§2.10) | Boundary охраняет *занятое* окно одной activity; event sub-process охраняет *открытое* окно области (иное время жизни), ловит *откуда угодно изнутри* и работает *в* контексте данных области. Разделяемый **бюджет** прерывающих — единственное пересечение — смоделирован как разделяемый per-scope-реестр, а не схлопыванием двух конструкций. |
| **Прерывающий + непрерывающий в одном event-sub-слайсе** (§2.10) | Непрерывающий добавляет время жизни конкурентных обработчиков + взаимодействие со scope-drain; прерывающий — общий случай исключения и приземляется первым, непрерывающий — вторым, на том же взведении (решение владельца). |
...
- **Контрактное тестирование.** Вызванный процесс — это интерфейс: его
  ...
  [ADR-040 v.1](../ADR-040-process-io-contract.md) §2.4 проверяет объявленные
  имена Call Activity против разрешённого callable **при запуске**
  (latest-at-launch означает, что пара известна только тогда); документированный
  путь deprecation для callables (зарегистрировать новую версию → мигрировать
...
| v.1 | 2026-07-17 | Руслан Габитов | **Принято** — оба слайса приземлены (встроенный Sub-Process и Call Activity, через сопровождающие SRD); эпик #85 закрыт. Только флип статуса, без изменения концепции (без bump версии). |
| v.2 | 2026-07-17 | Руслан Габитов | **Черновик** — добавляет §2.10 **Event Sub-Process**, продвигая шов §2.8 к полноценной концепции (эпик #91): `triggeredByEvent`-обработчик, **взведённый пока открыта его охватывающая область** (паттерн boundary-watch, поднятый с окна activity на окно области), переиспользующий per-kind-машинерию триггеров (hub Message/Signal, Timer, **conditional start** из ADR-006 v.3, приземляющийся здесь, обход Error §2.6); **прерывающий** старт отменяет сиблинг-треки своей области (§2.5) и запускает обработчик в контексте данных родителя (Error→Failing / не-error→Terminating реализованы путём отмены + наблюдаемостью, без нового состояния токена); **бюджет прерывающего — один на Event Declaration, разделяемый с граничными событиями** (§10.5.6), обеспечиваемый per-scope-реестром прерывающего взведения; **absorb vs re-throw** (§10.5.6) даёт контроль терминал-vs-декоратор. **Непрерывающий** решён (конкурентный спавн обработчика, без ограничения, Error исключён), но реализация нарезана вторым слайсом. **Transaction/компенсация + Escalation-триггер остаются вне области** — им нужен #90; §2.8 держит их как «спроектировано под». Bullet event-sub §2.8 обновлён на указание §2.10; §3 grounding + §4 альтернативы + Engine-notes расширены. Краеугольный камень v.1 (§2.1–§2.7) не изменён и остаётся принятым. Grounded против §13.5.4 / §10.5.2 / §10.5.6 / §10.4.3. Реализация сопровождающим SRD (прерывающий первым). |
| v.2 | 2026-07-18 | Руслан Габитов | **Принято** — прерывающий Event Sub-Process приземлён (сопровождающий SRD): взведённый областью обработчик, cancel-and-run, разделяемый с граничными событиями бюджет прерывающего, catch по цепочке областей для Error и absorb. Непрерывающие обработчики и Transaction/компенсация остаются нарезанными (#90). Только флип статуса, без изменения концепции (без bump версии). |
| v.3 | 2026-07-22 | Руслан Габитов | **Черновик** — заметка §2.3 закреплена: `startQuantity`/`completionQuantity` ≠ 1 переквалифицированы из «существующей отложки» в **осознанный non-goal** (→ SAD-001 v.1 §4 N8) — неявное размножение/слияние токенов без нотации на диаграмме непрозрачно, Camunda их не поддерживает, а явный Parallel Gateway покрывает намерение видимо; runtime соблюдает дефолтную 1. Только документация; модель выполнения не изменена (§2.1–§2.10 без изменений). Расширен в черновике 2026-08-06 **контрактом рестарта** в §2.7 (выбор движка; стандарт молчит о рестартах): дочерний экземпляр долговечен сам по себе, чекпоинт вызывающего записывает вызов в полёте, восстановление поднимает оба конца и пересвязывает их, а отсутствующая запись контрагента громко проваливает восстановление (без тихого перезапуска ребёнка, без ребёнка-сироты) — управляется ADR-033 v.4 §2.10. RU twin синхронизируется при повторном принятии. |
| v.3 | 2026-08-08 | Руслан Габитов | **Принято** — контракт рестарта приземлён сопровождающим SRD (#277): долговечные дети с симметричной связкой родитель↔ребёнок, пересвязка при восстановлении (резидентная, in-flight и терминальная формы ребёнка), громкий отказ при отсутствующем контрагенте. Флип статуса; RU twin синхронизирован. |
| v.4 | 2026-08-26 | Руслан Габитов | Сторона вызываемого в I/O-отображении §2.5 теперь решена в другом месте и указана: собственные объявленные входы/выходы процесса, привязка при запуске, чтение при завершении и проверка параметров вызывающего по именам против разрешённого callable при запуске — [ADR-040 v.1](../ADR-040-process-io-contract.md). Рекомендация «registry-time-валидация» из заметки о контрактном тестировании заменена этой проверкой при запуске. Только указатели; семантика исполнения этого ADR не меняется. RU twin синхронизирован. |
