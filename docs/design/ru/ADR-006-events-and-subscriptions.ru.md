...
| Статус | Принято |
| Версия | v.5 |
| Дата | 2026-08-08 (v.4 принято 2026-07-20; v.5 принято 2026-08-08) |
...
> (§2.5); события Error (§2.6); события Conditional (§2.7); события Link
> (§2.8 — интра-процессный GOTO по статическому спариванию имён, единственное
> «событие», которое не является подпиской); и контракт доставки внутри
> экземпляра (§2.9, v.5 — связывание payload per-delivery, кратность по виду
> среди конкурентных ожидающих одного экземпляра, message-корреляция
> гранулярности итерации). Реализация едет с
...
### 2.9 Доставка внутри экземпляра: payload связывается per-delivery; кратность — по виду (v.5)

Один экземпляр может держать **несколько конкурентных выполнений, ожидающих
одно определение события на одном catch-узле** — канонический производитель
— параллельная Multi-Instance-активность, каждая итерация которой ждёт на
своём catch (каждая итерация — собственное выполнение над единственным
графом узлов экземпляра, ADR-009 v.1; per-instance параллельные области по
ADR-025 v.2 §2.2). Эта подсекция решает, как доставка достигает нужного
ожидающего и доносит payload целым.

**2.9.1 Payload — свойство доставки, никогда — узла.** Сработавшее
определение события несёт собственный item. Потребитель этого item'а —
**принимающее выполнение**: при доставке оно захватывает item из вручённого
ЕМУ определения и складывает в **собственный кадр выполнения**; catch-узел
связывает выходы из этого кадра. Узел — *определение*, неизменяемое в
рантайме: никакой изменяемый слот payload'а на узле не живёт, разделяемый
или нет. N конкурентных доставок в N выполнений складывают N item'ов в N
кадров; окна, в котором одно выполнение увидело бы payload соседа, не
существует. (Уточняет «waiter несёт payload к узлу» из ADR-014 v.1 §2.5:
адрес payload'а — **доставка**; узел лишь объявляет, какие выходы
существуют.)

**2.9.2 Кратность по виду.** Когда несколько выполнений одного экземпляра
ждут подходящее определение на одном узле, одно срабатывание обслуживает их
по семантике своего вида — зафиксировано как контракт, а не оставлено
механизму:

| Вид | Одно срабатывание обслуживает |
|---|---|
| Message | **ровно одного** ожидающего — скоррелированного (2.9.3) |
| Signal | **каждого** ожидающего (fan-out публикации — `event-handling.md`: триггер «MAY be received by any catching Event in any scope where the trigger is published») |
| Timer | собственную регистрацию (per-execution-расписания; ничего не разделяется) |
| Conditional | по §2.7 (владеет loop; каждая взведённая подписка вычисляется) |

Стандарт коррелирует Message к **экземпляру Process** (§8.4.2 — «Message
routed to that Conversation (and the Process instance behind it)» из
extract'а) и **молчит** о выборе среди токенов внутри него; «ровно один»
для message и «каждый» для signal — выборы движка: первый реализует
замысел корреляции на более тонкой грануле, второй — процитированную
семантику публикации.

**2.9.3 Message-корреляция гранулярности итерации.** Подписка каждого
конкурентно-ожидающего выполнения несёт **ключ корреляции, выведенный из
split-item'а его итерации** (датум, который ADR-025 v.2 §2.6 связывает
per-instance); message-waiter сопоставляет envelope **ровно одной** подписке
через машинерию per-subscription-ключей (ADR-016 v.1; шов ключей ADR-014
v.1 §2.6). Параллельный MI message-catch, **не** объявивший корреляцию
итерации, **отвергается при регистрации**: с N одинаковыми подписками
доставка подлинно неоднозначна, и громкая ошибка моделирования лучше
произвольного выбора (позиция uncaught-events — ничто не маршрутизируется
молча).

**2.9.4 Жизнью waiter'а владеет очередь.** Waiter обслуживает **очередь
подписок** — регистрация добавляет пару (подписчик, определение),
дерегистрация убирает ровно эту пару, и waiter разбирается только когда его
очередь **опустела** — никогда по первому срабатыванию. (Уточняет
единоличное владение хаба из §2.5 правилом кратности: message-waiter,
убирающий себя после первого совпадения, осиротил бы подписки
итераций-соседей.)
...
|---|---|---|---|
| v.5 | 2026-08-08 | Руслан Габитов | **Принято** (приземлено через SRD-085: payload в кадре, msgSub-маршрутизация, WithIterationCorrelation, жизненный цикл waiter'а во владении очереди; make ci зелёный, diff-coverage 95.9%). Добавлена **§2.9 «Доставка внутри экземпляра»**: связывание payload **per-delivery** (принимающее выполнение захватывает item сработавшего определения в собственный кадр; узел — неизменяемое определение, слот на узле сконструирован прочь; уточняет ADR-014 v.1 §2.5), **таблица кратности по виду** (message → ровно один, скоррелированный; signal → каждый; timer → per-registration; conditional → §2.7), **message-корреляция гранулярности итерации** (per-subscription-ключи из split-item'а итерации поверх машинерии ADR-016 v.1; параллельный MI message-catch без корреляции итерации отвергается при регистрации) и **владение жизнью waiter'а очередью** (разбор по опустевшей очереди, никогда по первому срабатыванию; уточняет §2.5). Стандартное обоснование: §8.4.2 коррелирует к экземпляру (цитата); выбор токена внутри — молчание стандарта, выборы движка названы явно. Мотивировано #305. Реализация — сопровождающими SRD. |
