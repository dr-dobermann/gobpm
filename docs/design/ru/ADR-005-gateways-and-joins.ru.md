...
**собственному, gateway-level условию**, а не только по условиям flow. Он **требуется
Process Execution Conformance наравне с остальными шлюзами**: §13.4.5 задаёт ему
полную операционную семантику, а BPMN 2.0.2 §2.3.1 обязывает соответствующий
инструмент «полностью поддерживать и интерпретировать операционную семантику» —
никакого подмножества, выше которого можно сидеть, не существует. Он к тому же
выражает паттерны, недоступные остальным четырём: **Structured / Blocking
Discriminator** (WCP-9 / WCP-28) и **Structured / Blocking Partial Join**
(WCP-30 / WCP-31) — что объясняет, почему он *ценен*, а не почему он
*необязателен*.

> **Исправлено в v.5.** v.1–v.4 описывали этот шлюз как «явное расширение над
> Common Executable Subclass (conformance §2.1.3)». Обе половины неверны. §2.1.3
> не существует, а Common Executable Subclass — это подкласс Process
> **Modeling** Conformance (§2.2.1), предназначенный для инструментов, которые
> *порождают* исполняемые модели, и требующий XML Schema, WSDL и XPath — он
> никогда не был целью этого движка. Шлюз требовался всё это время. Поведение не
> меняется: эта версия исправляет обоснование, а не контракт.
...
- OR-join Camunda 7 послужил engine-прецедентом: его внутренний
  reference-анализ информировал консервативную, двухуровневую,
  переоцениваемую-при-смерти-токена реализацию §2.10 и отклонения, которые
  gobpm умышленно сохраняет или исправляет.
...
| v.5 | 2026-08-02 | Руслан Габитов | Принято. **Только исправление — контракт не меняется.** §2.11 описывал Complex-шлюз как «явное расширение над Common Executable Subclass (conformance §2.1.3)». Обе половины неверны. §2.1.3 не существует: §2.1 — *General*, §2.2 — Process **Modeling** Conformance, а Process **Execution** Conformance — это **§2.3**; ошибочная ссылка прослеживается до самого параграфа перекрёстных ссылок §2.1, который сдвинут на единицу относительно заголовков спецификации. И **Common Executable Subclass — подкласс Process *Modeling* Conformance** (§2.2.1), предназначенный для инструментов, *порождающих* исполняемые модели, и требующий XML Schema, WSDL и XPath — он никогда не был целью движка, поскольку gobpm по замыслу использует Go-типы, Go-операции и goexpr/Lua. Следствие для этого ADR: Complex-шлюз **не расширение**. §13.4.5 даёт ему полную операционную семантику, а §2.3.1 обязывает соответствующий инструмент «полностью поддерживать и интерпретировать» её, так что он требовался всегда; паттерны WCP-9/28/30/31 объясняют, почему он *ценен*, а не почему был *необязателен*. Поведение, валидация и runtime-протокол шлюза не тронуты — каждое предписание §2.11 остаётся как написано, поэтому входящие пины на v.4 остаются корректными. Исправляет ту же семью опечаток, что и ссылка §10.3.4.1 на Таблицу 8.49; сама scope-авторитетность исправлена в `docs/bpmn-spec/conformance.md` и [SAD-001](../SAD-001-vision-and-architecture.md) §14. |
