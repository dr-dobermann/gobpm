...
Чего слой **не умел** по состоянию на v.6 — пробел, который закрыла §2.9 — это
дотянуться *внутрь* значения. Структура `ItemDefinition` была **непрозрачна**:
значение было плоским — скаляр (`Variable`) или однородный список (`Array`), без
record-вида и без видимой движку формы, — так что mapping, associations,
выражения и условия могли адресовать *целое* значение, но не
`order.items[0].price`, и не могли собрать вложенный выход (§2.9).

**(v.7)** Структурные срезы S1–S4 с тех пор приземлились: значения навигируемы
(`Record` рядом с `Collection`), записываемы через пути, изменения
обнаруживаются commit-diff'ом, а нативные Go-структуры участвуют через
кэшированные адаптеры. Чего набору видов всё ещё не хватает — это
**словаря с data-ключами**: в `scalar｜list｜record` нет вида, чьи ключи — *данные*,
а не схема, так что процесс не может держать словарь, растущий ключ-за-ключом, а
нативное struct-поле `map[string]V` участвует лишь как непрозрачный,
ненавигируемый скалярный лист (умышленный fallback S4-builder'а). §2.9.7 это
закрывает.
...
§2.9 это закрывает. **(v.7)** Конкретный драйвер, которого §2.8 велела ждать
map-виду, прибыл: процессы действительно держат словари с data-ключами
(накопление результатов под runtime-ключами), а приземлённый S4-адаптерный слой
делает пробел `map[string]V` видимым на практике — нативное map-поле — та самая
распространённая Go-форма, которую движок не может навигировать; §2.9.7 это
решает. По нашему постоянному принципу — более ранний документ
...
**Чем может быть каждое выражение (v.10).** §2.4 назвал три формы; он не сказал,
к чему адресуются выражения в них, а две половины `assignment` — не одно и то же
по природе.

- **`transformation`** и **`from`** у assignment — обычные выражения: вычисляются
  через `ExpressionEngine` в **data-контексте activity** — том же резолвере, через
  который читает всякий другой потребитель (§2.9.2), так что mapping-выражение
  видит source'ы association, собственные данные узла и структурные пути внутрь
  любого из них, а `order.items[0].price` разрешается ровно так же, как в условии
  gateway. Результат — значение. **Source'ы** association решают, может ли она
  вообще выполниться (гейт доступности ниже); они не сужают то, что видит её
  выражение — §10.4.2 ограничивает гейт, а не видимость.
- **`to` у assignment — путь к данным, а не произвольное выражение.** Он
  адресует, куда ложится значение — сам target association (путь без шагов, то
  есть запись значения целиком) либо поле или элемент внутри него — и
  разрешается тем же обходом, записывая по правилу §2.9.3: обход доходит до
  **родителя** и мутирует через его capability, так что владелец сам обеспечивает
  свою форму (типизированное значение отвергает неизвестное поле по построению,
  динамическая запись принимает собранное), а нарушающая запись — ошибка, а не
  тихое приведение.

Сужение — **выбор движка**, и выбор осознанный. §10.4.2 определяет `to` как
Expression, вычисляющееся в «любой элемент в контексте или его подэлемент», что в
общем случае требует вычислителя, дающего **место**, а не значение — второго
контракта, lvalue-образного, поверх единственного expression-шва этого движка.
Сужение не стоит ничего из того, за чем тянется моделировщик: путь уже адресует
всякий элемент и подэлемент, которые держит движок (§2.9.2/§2.9.3 существуют
именно для этого), и **практика согласна** — input/output-mapping Camunda 7,
основной заменитель этих форм, берёт целью простое **имя** переменной
([camunda7/input-output-mapping.md](../../camunda7/input-output-mapping.md) §1).
Путь — строго больше, чем это имя, и строго меньше, чем lvalue-вычислитель.
Документ, чей `to` не является разрешимым путём, отвергается там, где читается, и
никогда не копируется целиком.

**Одна форма за раз.** Association несёт `transformation` **либо** `assignment`'ы,
никогда оба. §10.4.2 нумерует три случая, не запрещая комбинацию, и его правила
скомпоновались бы — transformation замещает target, затем каждый assignment пишет
в результат — но ничто в стандарте не фиксирует этот порядок, никакая практика его
не отрабатывает
([camunda7/input-output-mapping.md](../../camunda7/input-output-mapping.md) §2), а
mapping, чьи две половины дерутся за один target, — ошибка моделирования, которую
стоит поймать. Это **выбор движка**, отвергаемый там, где association строится.
Направление обратимое: допустить комбинацию позже — вопрос одного решения, тогда
как выдумать порядок сейчас — зафиксировать тот, которого никто не просил.

**Source'ы следуют форме.** Без обеих форм-выражений легален **ровно один**
source — собственное правило 3 стандарта
([data.md](../../bpmn-spec/semantics/data.md) §10.4.2, p225) — почему
multi-source association законна лишь в обществе transformation. При
transformation source'ы гейтят доступность независимо от того, называет ли их
выражение (правило 1: «source'ы используются как гейт доступности в scope, но не
обязаны упоминаться внутри Expression»), а тело спецификации ставит их
кардинальность «от нуля (0) до многих» — эту оговорку вендорённый extract не
несёт, она процитирована целиком в документации пакета
`pkg/model/data/association.go`. Сам гейт доступности этот раздел не меняет:
недоступный source падает быстро, а не ждёт, по §2.3.
...
required-входом есть ошибка во время срабатывания. Половина уровня процесса —
собственные `DataInput`'ы и `DataOutput`'ы процесса как объявленный контракт,
привязываемый при запуске и читаемый при нормальном завершении, — решена в
[ADR-040 v.2](../ADR-040-process-io-contract.md). Особый случай Start/End
(process `DataInput`'ы как targets output-associations Start-события; process
`DataOutput`'ы как sources input-associations End-события) решён там же
(§2.7): data-параметры события — это концы ассоциаций, к которым
подключаются Data Object, Data Store или собственный контракт процесса, а
запуск, рождённый триггером Start-события, заполняет контракт из payload'а
до его проверки.
...
- **Отображение вендорского I/O-mapping'а на эти формы (v.10).** Раз association
  вычисляет выражения, у вендорской конструкции, делающей то же самое —
  `camunda:inputOutput` Camunda 7, — впервые появляется дом в этой модели.
  Отображает ли её конвертер, здесь **не решается**: это вопрос диалекта
  конвертера, управляемый правилом
  [ADR-024](../ADR-024-process-interchange-converters.md) §2.14 — диалект никогда
  не мотивирует новый тип модели, а отображает то, что уже есть. Этот ADR лишь
  делает дом существующим.
...
- **(v.7) Нативные map'ы с не-строковыми ключами.** Map-вид (§2.9.7) делает
  struct-поля `map[string]V` навигируемыми; Go-map с любым **другим** типом ключа
  (`map[int]V`, `map[K]V` над comparable-структурой, …) остаётся тем, чем является
  сегодня — непрозрачным, цельно-значным скалярным листом в адаптерном слое.
  **Стрингификация** ключей отвергнута: это тихая коэрция (против fail-loud-позиции
  §2.9.3), неоднозначная по формату (`1` vs `01`, отрицательные числа, составные
  ключи) и необратимая на пути записи. Тип, которому нужен навигируемый map с
  не-строковыми ключами, поднимает себя per-type через custom-adapter-хук (реестр
  §2.9.5), а не через engine-wide-правило коэрции. *(Сам map-вид, отложенный здесь
  до v.6 включительно «до появления конкретного драйвера», решён в §2.9.7 —
  драйвер прибыл: см. §1.3.)*
...
- **map (v.7)** — `Value`, реализующий способность **`Map`**: однородные значения
  под **данными**-ключами — произвольными строками, не идентификаторами-именами
  полей — с детерминированным перечислением ключей, чтением/записью entry и
  полноправным **удалением** entry (§2.9.7). По-настоящему отличный вид от record:
  ключи record'а — его *схема*; ключи map'а — *данные*.

Набор видов — эти четыре, и он закрыт, пока решение здесь его не переоткроет.

*Вид* узла — это **какую способность он реализует** (один type assertion);
`Value` обязан реализовывать **не более одной** структурной способности —
kind-пробы проверяют в одном документированном, фиксированном порядке, и модель
не определяет значения, являющегося одновременно record'ом и map'ом. Тип
...
один новый конкретный тип — сохраняющий порядок вставки **`values.Record`**
...
вкладываются друг в друга. **(v.7)** Map-вид поставляет свой собственный
динамический конкретный тип — обобщённый **`values.Map[T]`**, зеркалящий
`Array[T]`: однородность форсируется параметром типа, а `Map[any]` — словарь
нулевой настройки для собираемых движком данных (§2.9.7).
...
индексирует **list**, а **(v.7)** `["key"]` — строка в двойных кавычках с
backslash-эскейпом внутри скобок — выбирает entry **map'а**
(`rates["EUR"].value`). Скобочные формы не сталкиваются: голое число — индекс
списка, quoted-строка — ключ map'а (`[0]` индексирует список; `["0"]` адресует
ключ map'а `"0"`). Критически это свойство **доступа к данным** — резолвера
...
закрывая пробел записи, который был у модели до v.6, где mapping и associations
могли замещать только **целые**
...
`values.Record` пермиссивен и принимает собранные поля. **(v.7)** Запись в
**map** пермиссивна по *ключу* по определению — ключи это данные, так что
`SetPath` по `rates["EUR"]` делает upsert entry — тогда как сторона *значения*
остаётся форсируемой владельцем (типизированный `values.Map[T]` или
адаптированный `map[string]V` отвергает несовпадение типа значения).
Отсутствующие промежуточные
records на пути создаются, когда target это позволяет — **(v.7)** шаг, за которым
следует `["key"]`, vivify'ит пустой динамический map, ровно как следующий
`.field` vivify'ит record, а следующий `[i]` — list; нарушающая запись —
...
- **(v.7) S5 — map-вид.** Способность `Map` и динамический `values.Map[T]`;
  шаг пути `["key"]` на чтении, записи и vivify; per-entry-обход commit-diff'а;
  лифт `map[string]V` в адаптерном слое (§2.9.7). Аддитивен над S1–S4 — ни одна
  приземлённая поверхность не меняет форму; S4-fallback «map-поле — непрозрачный
  лист» сужается до map'ов с не-строковыми ключами (§2.8).

ADR-011 v.6 **принят** (Accepted), как только S1 доказывает модель; S2–S4 уточняют
против неё. **(v.7)** S1–S4 приземлились; S5 едет тем же паттерном — одна
концепция здесь, приземляемая целиком сопровождающим maps-SRD.

#### 2.9.7 Map-вид — однородные значения под data-ключами (v.7)

v.6 признал map отличным, аддитивным будущим видом и припарковал его в §2.8 до
появления конкретного драйвера; драйвер прибыл (§1.3), и эта секция его решает.
**Map — это словарь, чьи ключи — *данные*** — произвольные строки, порождаемые в
runtime — тогда как ключи record'а — его *схема*. Это одно различие движет каждым
выбором ниже.

**Способность.** `data.Map` присоединяется к `Record` / `Collection` как
четвёртый вид (§2.9.1): перечисление ключей (`Keys`), чтение entry, upsert entry
и **удаление** entry — удаление полноправно *потому что* ключи — данные (record
никогда не теряет поле схемы в runtime; map законно теряет entry). Значения
**однородны** — одна форма значения под всеми ключами, контракт Go-map — и сами
могут быть records, lists или maps: вложенность композируется как везде.

**Детерминизм поверх рандомизированной итерации Go.** Порядок итерации Go-map
намеренно случаен; перечисляющие поверхности движка такими быть не должны. Каждая
поверхность, перечисляющая entries map'а — `Keys`, shape/walk-хелперы и прежде
всего commit-diff — перечисляет в **сортированном порядке ключей**. У map'а нет
порядка вставки, который стоило бы сохранять (в отличие от `values.Record`, чей
порядок — порядок его схемы); сортировка — единственный канонический порядок,
стабильный между клонами, коммитами и рестартами процесса, так что диффы,
`DataChange`-факты и вывод walk'а воспроизводимы.

**Цельно-значная семантика следует однородным видам.** Динамический
`values.Map[T]` зеркалит `Array[T]` (§2.9.1): `Get` снапшотит в `map[string]T`;
`Update` **заменяет** набор entries на переданный — `Get`/`Update` образуют
round-trip, и замена может выразить удаление. Merge-позиция `values.Record` на
`Update` *не* переносится: record мержит, потому что неизвестные ключи — это
нарушения схемы, которые надо отвергнуть; у map'а нет неизвестных ключей, так что
«merge» молча стал бы upsert'ом, который никогда не может удалять — хирургия
принадлежит entry-операциям, оптовая замена — `Update`.

**Путь, запись и diff.** `["key"]` — map-шаг в единой грамматике путей (§2.9.2) —
чтение, запись (upsert + vivify, §2.9.3) и условия все идут через один резолвер.
Commit-diff (§2.9.4) обходит map как record — per entry, объединение старых и
новых ключей, эмитя `(path, ChangeType)` с `Value_Added` / `Value_Updated` /
`Value_Deleted` на ключ — так что изменения map'а питают `KindDataChange`-факты и
субстрат conditional-events без нового словаря.

**Нативный `map[string]V` участвует через тот же адаптерный шов** (§2.9.5):
reflection-builder классифицирует string-keyed-map-поле как map-вид и обслуживает
его через кэшированный адаптер поверх **живого** map'а — wrap, не convert — ровно
как структуры и слайсы уже едут на `Record` / `Collection`. Не-строковые типы
ключей остаются непрозрачными листами (§2.8). Это сужает умышленный S4-fallback
«map-поле — непрозрачный лист» до форм, которые действительно не могут нести
path-ключ. Одна **engine note**: значение map'а host-языка может быть
не-*адресуемым* (в Go — не адресуемо), поэтому для нативного map'а контракт
записи — **entry-level**: значение целиком под ключом заменяется или удаляется,
вживую — тогда как *составное* значение, прочитанное из нативного map'а, — это
навигируемый **снимок** (глубокая запись в него падает громко, а не мутирует
отсоединённую копию). У динамического `values.Map` такого ограничения нет; это
ограничивает лишь native-interop-слой, и приземляющий SRD это фиксирует.

**Заземление в стандарте.** Собственной модели данных BPMN словарный вид не нужен
— `ItemDefinition.structureRef` указывает на «конкретную структуру данных, обычно
XSD complex type или element» (§8.4.10, Table 8.47), а XSD композирует records и
lists; о словарях с data-ключами стандарт молчит. Map-вид — таким образом
**engine choice** — удобство Go-interop и моделирования, не conformance-предмет —
фиксируемый как таковой, подобно прочим умышленным engine choices, в
engine-choices-охвате SAD-001 при приземлении.
...
- **(v.7) Набор видов полон для Go-образных данных.** С map-видом (§2.9.7)
  каждая распространённая Go-форма данных — скаляр, слайс, структура,
  string-keyed map, вложенные как угодно — участвует в путях, условиях,
  mapping'ах, записях и обнаружении изменений; единственный умышленный остаток —
  нативный map с не-строковыми ключами (§2.8), поднимаемый per-type через
  custom-adapter-хук.
- **(v.7) Цена: четвёртый вид протягивается через каждую kind-probe-поверхность.**
  Грамматика путей получает quoted-key-шаг; классификатор видов,
  shape/walk-хелперы, write-walker с его vivify-правилом, commit-diff и
  adapter-builder каждый получают по одной map-ветке — аддитивно на каждом шве
  (ни одна приземлённая поверхность не меняет форму), приземляется срезом S5
  (§2.9.6), держа `make ci` зелёным.
...
- Нет. Охват одного набора (множественные I/O sets — не-цель), семантика
  ...
  приземляющего(их) SRD, а не открытые концептуальные вопросы. **(v.7)** Map-вид
  равным образом решён (§2.9.7): форма способности, сортированное перечисление,
  replace-на-`Update`, грамматика `["key"]` с list/map-разведением скобок,
  полноправное удаление и адаптерный лифт только-`map[string]V` (без
  стрингификации ключей, §2.8). Точные сигнатуры `Map`, лексер quoted-шага и
  механика diff/vivify — забота приземляющего SRD.
...
|---|---|---|---|
| v.10 | 2026-08-28 | Руслан Габитов | **Принят** — приземлён SRD-097 в M1 (`data.Assignment`, `WithAssignments`, правила формы и кардинальности), M2 (expression-движок, достижимый из execution-frame), M3 (путь копирования, вычисляющий все три формы), M4 (ограничение legacy-вычислителя), M5 (импортёр, отображающий `<transformation>` и `<assignment>` и снимающий три отказа), M6 (рабочий пример) и M7 (находки независимого ревью). Сужение `to` до пути к данным из §2.4, правило «одна форма за раз» и кардинальность source'ов — всё проверяется там, где association строится. Два прочтения движка, к которым принудило приземление, зафиксированы в SRD-097 §10.2: data-контекст activity включает **собственные** output-параметры узла, и они выигрывают у одноимённого значения из scope; assignment, чей target — Data Store, читает запись перед тем, как записать её обратно, так что поля, которых он не называет, выживают. `make ci` зелёный. |
| v.10 | 2026-08-27 | Руслан Габитов | **§2.4 говорит, к чему адресуются выражения association.** Три формы были предписаны в v.1, а двум из них, управляемым выражениями, контракт так и не дали: `transformation` и `from` у assignment — обычные выражения, вычисляемые через `ExpressionEngine` в data-контексте activity, читающие source'ы по имени и по **пути**; **`to` у assignment — путь к данным**, разрешаемый тем же обходом и записываемый по правилу родительской capability из §2.9.3, так что владелец обеспечивает форму, а нарушающая запись даёт ошибку. Сужение «любого элемента в контексте» из §10.4.2 до пути — явный **выбор движка**: общая форма требует lvalue-образного вычислителя, шва под который у движка нет; путь уже адресует всё, что движок держит; и практика согласна (I/O-mapping Camunda 7 целится в простое имя переменной — вендорёно как `docs/camunda7/input-output-mapping.md`, чтобы утверждение можно было проверить). Выражения вычисляются в **data-контексте activity**, а не против одной лишь association: source'ы гейтят выполнение, но не сужают видимость. Association несёт transformation **либо** assignment'ы, никогда оба — выбор движка, поскольку стандарт не фиксирует порядок комбинации и ничто её не отрабатывает. Source'ы следуют форме: много их законно лишь при transformation, ровно один — без обеих форм (§10.4.2, правило 3), почему multi-source association ходит вместе с transformation. §2.8 фиксирует, что отображение вендорского I/O-mapping'а на эти формы — вопрос ADR-024 §2.14, а не этого ADR. §2.1–§2.3, §2.5–§2.7 и §2.9 не менялись. |
| v.9 | 2026-08-27 | Руслан Габитов | Особый случай Start/End из §2.5 больше не «остаётся запланированным»: он решён в [ADR-040 v.2](../ADR-040-process-io-contract.md) §2.7 — data-параметры события суть концы ассоциаций, собственные Start/End-события процесса подключаются к его контракту, event-born запуск заполняет контракт из payload'а. Только указатель; семантика data-flow не меняется. |
| v.8 | 2026-08-26 | Руслан Габитов | Разрешено «приземляется с работой по messaging/call-activity» из §2.5: I/O уровня процесса — собственные объявленные входы и выходы процесса, привязка при запуске, чтение при завершении — решён в [ADR-040 v.1](../ADR-040-process-io-contract.md); особый случай Start/End через event-associations остаётся запланированным, теперь против возможности присоединения данных к событиям (#329). Только указатель и перепланирование; семантика data-flow не меняется. |
| v.7 | 2026-07-19 | Руслан Габитов | **Принято (Accepted)** (приземлён целиком через SRD-047 — срез S5; пять майлстоунов, `make ci` зелёный, diff-coverage 98.9%). Добавлена §2.9.7 — **map-вид**, снимающий отложенность не-цели §2.8 «вид-значение map / словарь» теперь, когда прибыл заявленный ею драйвер (процессы, держащие словари с data-ключами; приземлённый S4-адаптерный слой, оставляющий поля `map[string]V` непрозрачными). Набор видов становится `scalar｜list｜record｜map` (§2.9.1): **`data.Map`** — четвёртая способность — однородные значения под **data**-ключами (ключи record'а — схема; ключи map'а — данные), с перечислением ключей, чтением/upsert'ом entry и **полноправным удалением** (ключи — данные, entries законно исчезают). **Детерминизм поверх рандомизированной итерации Go:** каждая перечисляющая поверхность (Keys, walk/shape-хелперы, commit-diff) использует **сортированный порядок ключей** — единственный порядок, стабильный между клонами/коммитами/рестартами. Динамический конкретный тип: обобщённый **`values.Map[T]`**, зеркалящий `Array[T]` (однородность параметром типа; `Map[any]` для собираемых данных); цельно-значный `Update` **заменяет** набор entries (merge-довод `values.Record` не переносится — у map'а нет неизвестных ключей для отвержения, а merge никогда не мог бы удалять). Грамматика путей получает шаг **`["key"]`** (double-quoted, backslash-эскейп) с разведением скобок — голое число = индекс списка, quoted-строка = ключ map'а; запись upsert'ит, а следующий шаг `["key"]` vivify'ит пустой динамический map (§2.9.2/§2.9.3). Commit-diff обходит map по entry (объединение ключей), эмитя существующий словарь `ChangeType` (§2.9.4). Адаптерный слой поднимает поля **`map[string]V`** до навигируемых map'ов поверх живого значения; **map'ы с не-строковыми ключами остаются непрозрачными листами** — стрингификация ключей отвергнута как тихая, формат-неоднозначная, необратимая коэрция (новая не-цель §2.8; per-type custom-адаптер — лифт). Заземление в стандарте: BPMN/XSD — record+list и молчит о словарях (§8.4.10) — map-вид — фиксируемый **engine choice**, не conformance-предмет. Фазирование получает **S5** (§2.9.6), аддитивный над приземлёнными S1–S4. Правит §1.2/§1.3/§2.8/§2.9.1/§2.9.2/§2.9.3/§2.9.6/§3/§6; добавляет §2.9.7. Приземляется через сопровождающий maps-SRD. |
| v.6 | 2026-07-13 | Руслан Габитов | **Принято (Accepted)** (концепция v.6; S1 — read path — приземлён сопровождающим wiring-SRD, доказывая модель по §2.9.6; S2–S4 — будущие срезы). Добавлена §2.9 **структурные данные — навигируемые значения**, закрывающая пробел «может адресовать целое значение, но не дотянуться *внутрь* него», обнажённый работой над service-task/mapping'ом. Заземлено на собственный `ItemDefinition.structureRef` стандарта («конкретная структура данных, обычно XSD complex type», §8.4.10). **Модель значений — по-Go-шному** (§2.9.1): существующее семейство `Value` получает один capability-интерфейс — **`Record`** (упорядоченные `Keys`/`Field`/`SetField`), зеркалящий существующий `Collection`; вид = какую способность реализует значение; вложенность композируется до любой глубины; один новый конкретный `values.Record` для динамических данных; **форма выводится обходом графа значений** (нет хранимого артефакта схемы); недоспецифицированные элементы остаются непрозрачными скалярами по §10.4.1. **Адресация по пути** (`order.items[0].price`) живёт в **шве доступа к данным** (§2.9.2) — один резолвер обслуживает mapping, выражения и **gateway/flow-условия**; согласовано с ADR-010 v.2 §2.7 (`/` выбирает провайдер, чужой+непрозрачный; `.`/`[]` обходят engine-managed-значения). **Чтение и запись** оба текут через путь (§2.9.3, форма, форсируемая владельцем: типизированные значения отвергают по построению, динамические records пермиссивны). **Обнаружение изменений — commit-diff** (§2.9.4 — scope диффит закоммиченные значения в набор `(path, ChangeType)`), выбранный против подписки на каждый `Value` (хрупкой под frame-clone-then-replace; commit — граница activity, на которую приземляются копии данных §10.4.2); он даёт дом фактам `KindDataChange`, отложенным ADR-013 v.2, является будущим субстратом для conditional events (ADR-006) и **удаляет дремлющую машинерию `Updater`/`UpdateCallback`** (ноль потребителей; `ChangeType` перенацелен как словарь диффа). **Native-struct-interop едет на реестре per-type-адаптеров** (§2.9.5): динамическим значениям адаптер не нужен; **reflection-builder на регистрации — стандарт** (reflect один раз на тип, кэшируется, вне пути выполнения — умышленное *ограниченное* послабление anti-reflection-позиции, фиксируемое как engine choice в SAD-001); **codegen-генератор — per-type-апгрейд** на том же шве (compile-checked, reflection-free, принимаемый позже без изменения движка); struct-теги `gobpm:"..."` конфигурируют лишь построение адаптера. Reflection на горячем пути остаётся отвергнутой. Фазировано **S1–S4** (§2.9.6 — принято, как только S1 доказывает модель). Соблюдает разделение value-vs-notification §2.7 и оставляет §2.2/§2.3 нетронутыми. Также обновлена §1.2 до post-v.5-текущего состояния (по правилу ревью current-state-freshness). Правит §1.2/§1.3/§2.1/§2.8/§3/§4/§6/§7; добавляет ссылки ADR-013 v.2, ADR-006 v.2. Приземляется через структурно-data SRD(ы). |
