...
[ADR-040 v.1](../ADR-040-process-io-contract.md). Особый случай Start/End
(process `DataInput`'ы как targets output-associations Start-события; process
`DataOutput`'ы как sources input-associations End-события) остаётся частью
концепции и приземляется вместе с возможностью присоединения данных к событиям
([#329](https://github.com/dr-dobermann/gobpm/issues/329)), которая ему нужна.
