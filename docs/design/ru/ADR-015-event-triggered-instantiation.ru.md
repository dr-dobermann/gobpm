...
**event-based gateway**, используемый на старте, и старт через
parallel-event-gateway — отложенные к §2.6 в v.1 — теперь реализованы (узел шлюза
появился в mid-flow-форме, затем последовали инстанциаторы Exclusive-start и
Parallel-start в workstream'е шлюзов).
...
[ADR-002 v.1](ADR-002-extension-architecture.ru.md) / ADR Distribution &
Scale). Нижняя граница по ограничению количества (no-OOM) непреложна;
стартер не поднимает ошибку
...
- **Инстанцирование через event-based-gateway** — реализовано в workstream'е
  шлюзов ([ADR-005 v.4](ADR-005-gateways-and-joins.ru.md) §2.12: узел шлюза плюс
  инстанциаторы Exclusive-start и Parallel-start). *(Отдельная веха реализации
  шлюзов в v.1.)*
...
с тех пор реализован в workstream'е шлюзов
([ADR-005 v.4](ADR-005-gateways-and-joins.ru.md) §2.12), а долговечность и
гарантии качества брокера остаются отложенными в названные follow-up.
