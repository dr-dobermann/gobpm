...
- **Link does not ride the subscription machinery — and does not motivate a
  `SubscriptionKey()` generalization.** The signal-events conception (§2.4/§2.5)
  parked a name-keyed-matching generalization "until Link lands, the second
  name-keyed event." That premise is now **retired**: Link is a static redirect,
  not a runtime name-matched subscription, so **Signal remains the only**
  name-keyed hub subscription and the generalization stays unjustified (one
  consumer). The `docs/backlog.md` item is re-scoped accordingly.
...
§2.13a removes everywhere else. Under one subscription the hub sees exactly
what it sees for a plain node: one processor, one definition, one entry, and
§2.9.4's pair identity needs no amendment at all.

Three problems disappear rather than being solved:

- **Re-arming per pass.** A wait is armed when a track ARRIVES at a node, and
  an in-place iteration never re-arrives, which is the defect that made an
...
| v.6 | 2026-08-10 | Ruslan Gabitov | **Draft.** Adds §2.9.5 — for an activity carrying loop characteristics the registered SUBSCRIBER is the decorator (ADR-025 v.3 §2.13): one processor identity per iterated activity, holding one subscription per waiting instance. Matching is untouched — the hub still resolves an envelope to exactly one subscription by correlation (§2.9.3) and serves a signal to every subscription (§2.9.2) — and so is the receiving execution, which still captures the payload into its own frame (§2.9.1); only the registered identity moves, and with it the responsibility to dispatch to the instance owning the matched subscription. The identity is what makes the set re-armable: a sequential iteration re-arms every pass, and after a restart every waiting instance must be re-armed before any delivery is accepted — not against loss, since the broker buffers an unmatched envelope until a matching subscription appears, but against a partially armed set matching an envelope into the WRONG instance — with each execution registering as itself, no party owns either, which is why the engine refuses an iterated WAITING activity today. The decorator holds ONE subscription per definition and fans an arriving delivery out to the instances waiting on it, rather than one subscription per instance: an ordinal discriminator in the hub's entry — the alternative first reached for — would put iteration vocabulary into registration, matching and unregistration, which is iteration knowledge in a driver (ADR-025 §2.13a). Under one subscription §2.9.4's pair identity needs no amendment. Dispatch is by the trigger's NATURE — Signal, Timer and Conditional reach every waiting instance (§2.9.2 already broadcasts a signal); a Message is point-to-point and reaches exactly one, the instance whose iteration correlation matches (§2.9.3) or, when instances are indistinguishable, the first still waiting IN ORDINAL ORDER, the only order available and therefore the specified one. Consequently a PARALLEL Multi-Instance over a Message catch requires an iteration correlation key: "first in ordinal order" is defensible where exactly one instance waits at a time and arbitrary where N do. Lifetime: registered while ANY instance awaits the definition. This is also what removes the arrival-armed defect that made an iterated waiting activity unsupported — a subscription scoped to the ACTIVITY outlives every pass, so there is no per-pass arming to forget, and a restart re-arms one subscription rather than N. §2.9's opening and §2.9.3 re-pinned to ADR-025 v.3, whose §2.2 replaced per-instance parallel scopes with a frame per iteration and a scope only where the activity is a scope host. Three stale claims refreshed at the bump, since a version re-asserts the whole document: scope-chain Error/Escalation propagation was written twice as future work ("lands with the sub-process/boundary workstream") when ADR-023 v.3 §2.6 has long decided it — both now delegate to it by pin, alongside pins for the two other delegations (ADR-007 v.2.1, ADR-033 v.5); and §1.1's "the runtime today moves a track to TrackWaitForEvent but the delivery edge is missing" describes the pre-ADR baseline this document removed, so it is re-tensed to say so rather than reading as current. |
