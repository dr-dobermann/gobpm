...
> Activity** (§2.7) as a **child instance** of a separately registered
> process, invoked through the versioned registry with the standard's
> direct I/O mapping. One concept carries both: the **scope** — the joint
...
> to a composite host). Boundary events on composites, the **scoped**
> Terminate End Event (§13.5.6), and the Error **scope-chain** walk that
> ADR-006 §2.6 modelled and ADR-018 deferred all become live here. The
> scope model is deliberately shaped to host what rides it next —
> event sub-processes (with the ADR-006 v.3 conditional start),
> Transaction, Ad-Hoc, multi-instance — without rework (§2.8).
> Implementation is sliced by the accompanying SRDs; scope is epic #85.
...
- **Event sub-processes** (#91): a handler container **armed while its
  enclosing scope is open** — the boundary-watch pattern shifted from an
  activity's window to a scope's window. Its triggered start reuses the
  existing trigger machinery per kind (hub waiters for Message/Timer/
  Signal; the loop-local conditional subscriptions for the **Conditional
  start ADR-006 v.3 already decided**; the §2.6 walk for Error). An
  interrupting handler start cancels the sibling tracks of its scope —
  §2.5's scope-cancel, reused. The interrupting-handler budget (one per
  Event Declaration, **shared with boundary events**) and the
  absorb-vs-rethrow precedence rules are that ADR's decisions.
...
| Event sub-process: single triggered start, event-instantiated, parent context, interrupting budget | §13.5.4 p.436-439; §10.5.2 p.241 ("An Event Sub-Process MUST have a single Start Event") — seams only, decided with #91 |
...
unresolved-error → instance fault (inherited from ADR-006 §2.6).
