...
| 11 | `DataObject` execution semantics ✅ + `DataObjectReference` (deferred) + `DataStore`/`DataStoreReference` ❌ | 🟡 | [#82](https://github.com/dr-dobermann/gobpm/issues/82) | **DataObject scope integration landed (SRD-063, ADR-030 v.1)** — a `DataObject` is a **per-instance scope-resident named container**: registered on a `Process` or an embedded `SubProcess` (name-keyed, duplicate-guarded), **seeded into the matching scope** at start (Process-level → root; SubProcess-level → child scope on open, disposed at close), **resolved by name** via the walk-up, with **bidirectional DataAssociation flow** routed through per-instance scope (task output → DataObject; DataObject → task input; the association is a shared declaration, the value is per-instance, so no retargeting). `DataObjectReference` is a **deliberate non-implementation** (SAD-001 §14.1 + BPMN-translation rules). `DataStore`/`DataStoreReference` (the engine-level infrastructure port, ADR-030 §2.5/§2.6) ride **SRD-064** — durable persistence is the future Persistence & State workstream |
