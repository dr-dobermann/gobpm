...
| Last verified | 2026-07-15, against master post-SRD-046 (code sweep, not asserted from memory) |
...
| Event definitions | `Message` (incl. instantiation + correlation), `Timer` (in-memory), `Signal` (throw/catch/broadcast/start), `Error` (end + boundary), `Terminate` | ADR-014/015/016, SRD-013…017, SRD-026, SRD-029, SRD-030 |
...
|---|---|---|---|---|
| 1 | `ConditionalEventDefinition` execution (start / intermediate-catch / boundary) | 🟡 type exists | [#89](https://github.com/dr-dobermann/gobpm/issues/89) | **The SRD-044 commit-diff `(path, ChangeType)` seam was built as its substrate** — re-evaluate on referenced-variable change; the cheapest remaining event row |
| 2 | `SubProcess` (embedded) — token-scope container, scoped data/events | ❌ | [#85](https://github.com/dr-dobermann/gobpm/issues/85) | The structural keystone: unblocks rows 3–5, 12 |
| 3 | `CallActivity` + `CallableElement` I/O + `InputOutputBinding` + `GlobalTask` variants | ❌ | [#85](https://github.com/dr-dobermann/gobpm/issues/85) | Boundary events on CallActivity explicitly in scope (§10.5.4) |
| 4 | Event Sub-Process (`SubProcess` with `triggeredByEvent=true`) | ❌ | [#91](https://github.com/dr-dobermann/gobpm/issues/91) | Needs row 2; Error-start lives here |
| 5 | `Transaction` + `CancelEventDefinition` execution | ❌ / 🟡 | [#91](https://github.com/dr-dobermann/gobpm/issues/91) | Needs rows 2 + 10 (compensation) |
| 6 | `StandardLoopCharacteristics` execution + `MultiInstanceLoopCharacteristics` + `ComplexBehaviorDefinition` + `completionQuantity` | 🟡 / ❌ | [#88](https://github.com/dr-dobermann/gobpm/issues/88) | The second structural pillar; `completionQuantity` deferral noted in SRD-046 NFR-4 |
| 7 | `ScriptTask` execution | 🟡 model-only | [#87](https://github.com/dr-dobermann/gobpm/issues/87) | Can ride the Expression Layer epic ([#74](https://github.com/dr-dobermann/gobpm/issues/74)) |
| 8 | `BusinessRuleTask` (DMN) execution | 🟡 model-only | [#87](https://github.com/dr-dobermann/gobpm/issues/87) | Needs a rule-engine extension seam (the ADR-002 pattern) |
| 9 | `EscalationEventDefinition` execution | 🟡 type exists | [#90](https://github.com/dr-dobermann/gobpm/issues/90) | Signal (same epic) already landed |
| 10 | `CompensateEventDefinition` execution + compensation `Association` semantics | 🟡 type exists | [#90](https://github.com/dr-dobermann/gobpm/issues/90) | The `Association` type exists (`pkg/model/artifacts`); semantics deferred |
| 11 | `LinkEventDefinition` execution | 🟡 type exists | [#90](https://github.com/dr-dobermann/gobpm/issues/90) | The hub `SubscriptionKey()` matching generalization was deferred to exactly this landing |
| 12 | Boundary-on-SubProcess/CallActivity + Error **scope-chain propagation** | ❌ | [#79](https://github.com/dr-dobermann/gobpm/issues/79) | Explicitly deferred from ADR-018 to 0.2.0; needs row 2 |
| 13 | `DataObject` execution semantics + `DataObjectReference` + `DataStore`/`DataStoreReference` | 🟡 / ❌ | [#82](https://github.com/dr-dobermann/gobpm/issues/82) | The `DataObject` model exists (`pkg/model/data_objects`) |
| 14 | Timer durability + hydration (in-memory works today) | 🟡 | [#84](https://github.com/dr-dobermann/gobpm/issues/84) | Rides the persistence work (ADR-007/009) |
| 15 | `AdHocSubProcess` | ❌ | [#92](https://github.com/dr-dobermann/gobpm/issues/92) | Needs row 2 |
...
1. **#89 Conditional events** — the substrate (SRD-044's changed-path seam)
   already exists; smallest effort, closes a full event-definition row.
2. **#85 Sub-Process + Call Activity** — the keystone: four other rows
   (4, 5, 12, 15) are blocked behind the token-scope container.
