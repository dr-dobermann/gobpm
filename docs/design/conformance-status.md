...
| Lanes (model-only) | `Lane`, `LaneSet` — carried by `Process` and `SubProcess`, nesting, registration-validated, **never executed**. The engine's only deliberate 🟡: §2.3.1 permits ignoring lanes at run time, while §2.3.2 and the converter's semantic round-trip require the model to hold them | SRD-076 |
