...
| 5 | `ScriptTask` execution | 🟡 seam landed | [#87](https://github.com/dr-dobermann/gobpm/issues/87) | **Seam + task landed (ADR-031 / SRD-064)**: the multi-engine Script Engine seam (enumerable format claims, core routing registry, repeatable `WithScriptEngine`, loud claim conflicts and unclaimed-format errors), the rebuilt `ScriptTask` (route + execute + per-name sorted output commits) and the `Script` facts — execution works with any registered engine. The batteries Lua interpreter (`adapters/lua`), the example and the row flip ride SRD-065. Decoupled from the Expression Layer (#74) per ADR-031 §1 — different seams |
