...
- Узел с возобновляемым in-flight состоянием (позиция таймера, подписка корреляции, частичное состояние активности) владеет формой этого состояния; персистентность доберётся до него через контракт состояния на узел (определён в Persistence ADR), а **не** храня изменяемое состояние на разделяемом определении узла (определения узлов разделяются между instance и track'ами и ОБЯЗАНЫ оставаться неизменяемыми).
