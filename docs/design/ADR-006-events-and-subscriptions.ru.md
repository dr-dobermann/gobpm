...
> (§2.5); события Error (§2.6); и события Conditional (§2.7). Реализация едет с
...
| **Старт Event Sub-Process'а** | **Запланированный дом для Conditional-старта** — приземляется **с Sub-Process'ами** (`sub-processes.md` перечисляет Conditional среди типов старта event-sub-process). В отличие от top-level-случая, event sub-process работает **внутри живого instance**: его стартовое условие легально ссылается на **данные объемлющего scope** (§10.4.3) и оценивается той же ре-оценкой по commit-diff, которую решает этот раздел, — новая поверхность данных не нужна. Решение зафиксировано сейчас; реализация едет с workstream'ом Sub-Process. |
