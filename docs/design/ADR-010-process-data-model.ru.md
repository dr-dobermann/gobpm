...
- **Именованные data sources** — read-only **провайдеры**, адресуемые
  **path-qualified**-именем `SOURCE/var` (напр. `RUNTIME/STATE`), резолвящимся в
  этом источнике **без обхода к родителю**. Источник никогда не пересекается с
  default scope, так что пользователь может держать свои `STATE`/`INSTANCE`.

`/` — **зарезервированный разделитель пути**, запрещён в именах данных, так что
разбиение однозначно.
