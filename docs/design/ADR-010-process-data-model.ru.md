...
> выполнения узла* (frame выполнения). Это парный документ к
...
flowchart TD
    ...
    R --> S1
    F1["Frame выполнения: трек A выполняет задачу T (входы / выходы / носители)"]
    F2["Frame выполнения: трек B выполняет задачу T (отдельный рабочий набор)"]
...
### 2.3 Одно выполнение узла работает на одном frame'е выполнения

Каждое выполнение узла получает **frame выполнения** — эфемерный рабочий
набор ровно одного выполнения одного узла одним треком:

- **Создаётся на старте узла.** Frame создаёт трек; загрузка входов
  (вычисление входящих data association) заполняет **экземпляры входных
  параметров** frame'а из контейнерного scope'а — актуальные на момент старта
  узла, по семантике копирования стандарта.
- **Изолирован на время выполнения.** Узел работает только со своим frame'ом:
  экземпляры входных и выходных параметров и носители выполнения (канал
  ответа пользовательской задачи, контекст вычисления шлюза) живут во frame'е
  или как локальные переменные исполняемого кода — никогда на узле, никогда
  под ключом-именем узла. Frame несёт то, что пересекает границы стадий
  ...
  пересекающие один узел, получают два frame'а; итерации цикла — по frame'у
  каждая; ветки multi-instance (будущее) — по frame'у на ветку.
- **Идентифицируется парой (track, node).** Трек выполняется последовательно,
  поэтому в любой момент существует не более одного живого frame'а на пару
  (track, node) — пара достаточна как ключ frame'а. Цикл, повторно посещающий
  узел, переиспользует ключ только после исчезновения предыдущего frame'а;
  ...
  сгенерированный id не нужны: идентичность живого frame'а позиционна, а
  *история* выполнения — забота следа токенов (ADR-001), не frame'а.
- **Коммитится по завершении.** Выгрузка выходов вычисляет исходящие data
  association и проталкивает выходы frame'а в контейнерный scope **одним
  атомарным батчем** (§2.2) — эффект frame'а «всё или ничего» относительно
  чтений других треков. Затем frame отбрасывается; ничто из него не переживает
  выполнение.
- **Сбойное или прерванное выполнение не коммитит ничего.** Когда выполнение
  узла падает или его трек прерван, frame отбрасывается незакоммиченным:
  контейнерный scope никогда не наблюдает частичный выход сбойного
  выполнения, и отброшенный frame не оставляет следов — никакого
  ...
  сбойного выполнения — она читает frame перед утилизацией: точка расширения
  жизненного цикла frame'а, а не изменение этого контракта.
- **Экземпляры параметров — per-frame.** Структуры входов/выходов, которые
  читает и заполняет выполнение узла, инстанцируются для frame'а из
  ...
  определения остаются на узле, экземпляры живут во frame'е.

```mermaid
sequenceDiagram
    participant T as Трек (горутина)
    participant F as Frame выполнения
    participant D as Data plane (контейнерные scope'ы)
    T->>D: чтение источников (старт узла, атомарно)
    T->>F: создать frame, заполнить экземпляры входов
    T->>T: выполнить узел против frame'а
...
- Узел трогает данные **только через контракты data-ролей**: контракт
  потребителя на входе (загрузить входы во frame) и контракт производителя на
  выходе (выдать выходы из frame'а) — узел реализует роль, только если у него
...
- **Трек вручает узлу его окружение выполнения.** Окружение, против которого
  выполняется узел, трек конструирует per-execution: оно открывает frame
...
- События следуют той же модели: входы throw-события загружаются в его frame
  при срабатывании; выходы catch-события коммитятся из его frame'а при
  триггере (стандарт даёт событиям одно-наборную, неожидающую привязку
  данных — frame её естественный носитель).
...
- **Долговечная персистентность / регидрация** scope'ов и frame'ов — будущий
  Persistence & State ADR; этот ADR лишь формирует in-memory единицы, которые
  будут сериализованы (контейнерные scope'ы; frame'ы выполняющихся узлов).
...
- **Конкретные не-`RUNTIME` провайдеры источников** (данные приложения /
  бизнес-данные, JSON-документы) — будущее; §2.7 фиксирует механизм, а не
  конкретные провайдеры.

### 2.7 Адресуемый доступ к данным: default scope и именованные data sources

*(Добавлено в v.2.)* Доступ на чтение охватывает два вида целей, различаемых по
**имени**, переданному в `GetData`:

- **Default scope** — дерево контейнерных scope'ов (§2.1). **Простое** имя
  (`"user_name"`) резолвится frame-first, затем обходом **вверх** до корня — это
  собственные персистентные данные процесса; пользователь именует переменные
  свободно.
- **Именованные data sources** — read-only **провайдеры**, адресуемые
  **path-qualified**-именем `SOURCE/address` (напр. `RUNTIME/STATE`), резолвящимся
  в этом источнике **без обхода к родителю**. Источник никогда не пересекается с
  default scope, так что пользователь может держать свои `STATE`/`INSTANCE`.

Path-qualified-имя **разбивается по первому `/`**: ведущий сегмент выбирает
источник, а **остаток — собственное адресное пространство провайдера** — data
plane передаёт его **дословно**, не разбирая. Так провайдер свободен именовать
свои переменные как угодно: плоское имя для `RUNTIME` (`RUNTIME/STATE`) или
точечный путь / JSONPath для JSON/бизнес-провайдера
(`BUSINESS/order.items[0].price`). `/` — **зарезервированный разделитель**, запрещён
в именах данных default scope, так что разбиение по первому `/` однозначно.

Источники **подключаемы**: провайдер резолвит переменную по имени и перечисляет
свои имена. Движок поставляет один — **`RUNTIME`** (read-only-переменные
движка/инстанса: `STARTED_AT`, `STATE`, `TRACKS_CNT` сейчас; `initiator`,
`tenant`, … по мере добавления) — и механизм открыт для других (данные
приложения/бизнес-данные, JSON-документы) без изменения вызывающих (§2.6).

Два хелпера обнаружения завершают поверхность чтения:

- **`GetSources()`** — доступные именованные источники (сейчас `["RUNTIME"]`;
  default scope не перечисляется, ему не нужен префикс пути).
- **`List(path)`** — имена переменных, доступных в источнике; `List("")` возвращает
  имена instance-переменных default scope.

Это модель доступа, через которую читают и публичный service reader
(сопровождающий data-flow ADR), и разрешение условий/gateway'ев; поэтому
runtime/instance-переменным не нужен специальный аксессор — они читаются по
своему пути, как любые данные.
...
- **Конкурентные выполнения одного узла становятся безопасными**: frame'ы
...
- **Персистентность получает свою единицу данных**: контейнерные scope'ы
  (долговечные) и frame'ы (выполняющиеся узлы) — ровно те единицы, которые
  сериализует чекпойнт.
- **Доступ к данным единообразен, обнаруживаем и расширяем** (§2.7): простые
  имена читают default scope (обход вверх); `SOURCE/var` читает именованный
  провайдер (без обхода); `GetSources`/`List` их обнаруживают. Runtime/instance-
  переменные движка становятся обычными адресуемыми по пути чтениями (без
  специального аксессора), а новые провайдеры источников (данные приложения/
  бизнес-данные) подключаются без правки вызывающих.
- **Цена миграции:** путь данных каждого вида узла меняется — загрузка и
  выгрузка переезжают с хранимых узлом путей на frame'ы, а per-execution
...
- **Правило сопровождения:** вид узла может держать ровно две вещи: свою
  ...
  **одного выполнения**: всё, что одно выполнение одним треком читает,
  заполняет или несёт, принадлежит frame'у этого выполнения. Тест
  ...
  данные frame'а (этот ADR). Новый вид узла, записавший данные frame'а в
...
- **Per-track scope'ы данных со слиянием на join'ах.** Даёт изоляцию, но
  ...
  контейнерный scope; изоляция принадлежит *frame'у выполнения* с его
...
- **Frame'ы, адресуемые в пространстве путей scope'а** (например,
  ...
  значения frame'а, нарушая семантику копирования/коммита. Отклонено: frame'ы —
...
- **Эксплуатационные метрики на data plane.** Единый авторитет данных должен
  открывать число scope'ов, число записей на scope и число frame'ов как
  метрики движка — неконтролируемый рост данных (процесс, копящий
  переменные; утечка неутилизированных frame'ов) должен быть виден до того,
...
- Нет. Разбиение конфигурации данных по видам узлов и конкретные сигнатуры
  API frame'а/scope'а — концерны реализации для приземляющего SRD, а не
  открытые решения.
...
- [ADR-005 v.2 Gateways & Joins](ADR-005-gateways-and-joins.md) —
...
- Persistence & State ADR *(будущий)* — долговечная сериализация контейнерных
  scope'ов и выполняющихся frame'ов; `DataStore`.
...
|---|---|---|---|
| v.2 | 2026-06-13 | Руслан Габитов | Добавлен §2.7 (адресуемый доступ к данным): доступ на чтение охватывает **default scope** (простые имена, обход вверх — собственные данные процесса) и **именованные data sources** (read-only подключаемые провайдеры, адресуемые path-qualified `SOURCE/address` без обхода). Path-qualified-имя разбивается по **первому** `/` — ведущий сегмент это источник, остаток — собственное адресное пространство провайдера (плоское имя для `RUNTIME`, точечный/JSONPath-адрес для JSON-провайдера), передаваемый дословно. `/` — зарезервированный разделитель, запрещён в именах данных default scope, так что источники не пересекаются с данными пользователя (можно держать свои `STATE`/`INSTANCE`). Движок поставляет провайдер `RUNTIME` (`STARTED_AT`/`STATE`/`TRACKS_CNT` сейчас; `initiator`/`tenant`/… позже); другие провайдеры (данные приложения/бизнес-данные, JSON) подключаются без правки вызывающих. Добавлены хелперы обнаружения `GetSources()` и `List(path)`. Следствие: runtime/instance-переменные становятся обычными адресуемыми по пути чтениями (без специального аксессора), используемыми единообразно service reader'ом и разрешением условий/gateway'ев. Приземляется через SRD-010. §2.1-§2.6 не изменены (без ренумерации). |
| v.1 | 2026-06-12 | Руслан Габитов | Полное авторство (разворачивает seed от 2026-06-11). Решает runtime-модель данных: персистентные данные в per-instance **контейнерных scope'ах** (корень процесса + будущие дети-подпроцессы; никаких track-scope'ов, никаких node-scope'ов); дерево scope'ов извлечено из Instance в выделенный **data plane** с атомарностью целых операций (устраняет класс гонок §1.2 аудита по построению); один **frame выполнения** на выполнение узла, с ключом (track, node) — трек последователен, поэтому не более одного живого frame'а на пару; per-frame экземпляры параметров, атомарный батч-коммит, отбрасывание незакоммиченным при сбое (без частичной выгрузки, без осадка в scope'е); узлы держат только *определения* данных + lifetime-состояние ADR-009 — `RegisterData`/хранимый узлом `DataPath` упразднены, трек строит per-execution окружение; **last-committed-wins** для конкурентных коммитов одной переменной. Вне области: модельная семантика data flow (отдельный ADR), DataStore + долговечная персистентность (Persistence ADR), подписки на изменения scope'а. Отклонено: только-починка-блокировок, операции с данными через loop, per-track scope'ы со слиянием на join'е, данные выполнения на узлах, frame'ы, адресуемые путями. |
| v.1 | 2026-06-12 | Руслан Габитов | **Принято**, приземлено через SRD-007 v.1 (M2–M5, `d30dd2c`…`6c86620`). Одна правка формулировки при приземлении (§2.3): носители выполнения живут во frame'е ИЛИ как локальные переменные исполняемого кода — frame несёт то, что пересекает границы стадий загрузка → выполнение → коммит; носитель, ограниченный одной стадией, per-execution по построению. Детали механики frame'а, закреплённые реализацией (порядок разрешения с выходами как неразрешаемыми целями записи; флип Ready на входах, заполненных ассоциациями), живут в SRD-007 §7 — они уточняют, а не меняют это решение. |
