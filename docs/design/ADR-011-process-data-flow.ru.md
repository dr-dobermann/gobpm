...
### 2.6 Operation полиморфна; in-process Go-код читает через узкий reader
...
(`##WebService`, `##Unspecified` или vendor-specific). gobpm делает `Operation`
интерфейсом с двумя видами:

- **Message operation (каноническая).** Модель стандарта: данные входят через
  `inMessage` operation (связанный с `DataInput`'ами activity через data
  associations) и выходят через `outMessage`; реализация видит **только своё
  сообщение** и отвязана от scope процесса. Это conformant-путь для внешних
  сервисов.
- **Go operation (gobpm-native).** In-process Go-functor, который получает
  **узкий публичный read-only data reader** и **возвращает свой результат** — без
  message-церемонии. Reader выставляет доступ на чтение **по имени** и **по
  item-definition id** над данными выполнения (process properties в области
  видимости и runtime-переменными движка), резолвя нормальным для выполнения
  обходом frame→container (ADR-010). Это умышленное расширение message-only-
  Operation стандарта, зарегистрированное в охвате conformance
  ([SAD-001 v.1 §14.2](SAD-001-vision-and-architecture.md)); `implementationRef`
  помечает механизм.

Делая `Operation` интерфейсом, мы держим каноническую message-модель **чистой** —
сервис, её использующий, не может дотянуться до scope процесса — давая при этом
Go-in-process-коду полноправный доступ к данным как **opt-in-вид**, а не reader,
прикрученный к каждой operation. Свойства reader'а:
...
property и runtime-переменную по имени и возвращает свой результат — так, как должен
уметь in-process Go-код — не компрометируя канонический message-контракт.
...
- **Service-код становится реальным потребителем данных, не ломая стандарт.**
  `Operation` полиморфна: каноническая message operation остаётся message-only
  (отвязанной, conformant), а Go operation вручает своему functor'у узкий
  публичный reader (process properties + runtime-переменные по имени) и берёт его
  возврат — ambient-доступ ограничен явным Go-видом.
...
- **Прикрутить reader к каждой operation** (форма §2.6 v.2). Даёт ambient-доступ на
  ...
  внешних/conformant. Отвергнуто в пользу полиморфного разделения (§2.6 v.3):
  каноническая message operation остаётся чистой; ambient-доступ ограничен явным
  видом Go operation, который стандарт уже санкционирует через engine-defined-
  механизм `implementationRef`.
