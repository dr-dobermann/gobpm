...
кардинальности стандарта (≥1 каждого). gobpm **не** моделирует несколько наборов или
упорядоченный data-driven-выбор между ними; множественные I/O sets — явная не-цель
(§2.8) по тому же принципу, что и §2.3 — набор, *выбираемый тем, какие данные
случайно доступны*, есть скрытое, не-диаграммное ветвление, и любая реальная
альтернатива моделируется gateway'ем или boundary event. Богатые внутринаборные
различения, которые определяет стандарт, сохранены; отброшен лишь много-наборный
*выбор*.
...
самодостаточным компонентом над I/O-графом — что держит дверь открытой: если когда-
то появится реальный спрос на множественные I/O sets, вариант с упорядоченным
выбором можно **добавить как расширение** к этому компоненту, не переформируя модель
данных. Концепция сужена до одного набора; структура не закрывает дорогу к большему.
...
- **Единоличное владение I/O-графом.** `InputOutputSpecification` — единственный
  владелец своих параметров и наборов; связь параметр↔набор **однонаправленна**
  (набор знает свои параметры; параметр не несёт обратных ссылок на каждый набор,
  которому принадлежит). Двусторонний, взаимно-ссылочный граф, обязанный синхронно
  держать межтиповые инварианты, заменяется единоличным владением с производными
  запросами. У мутации одна власть, а не две.
- **Вычисление наборов — отдельная забота от хранения.** **Вычисление input/output
  ...
  компонент над I/O-графом, а не методы, размазанные по типам хранения. Держать его
  отдельным — также то, что делает расширение под много-наборность (§2.8)
  добавляемым позже без переформирования структур данных, если оно когда-то
  потребуется.
...
- **Множественные input/output sets (не-цель — умышленное отклонение от BPMN).**
  ...
  optional/required и while-executing сохранены *внутри* одного набора, так что
  ничего практического не теряется. Слой модели (§2.7) сформирован так, что вариант с
  упорядоченным выбором можно **добавить как расширение**, если реальный спрос
  когда-то появится, не переформируя модель данных. Это отклонение и no-wait-
  отклонение (§2.3) зарегистрированы в охвате conformance движка
...
- **Service-код становится реальным потребителем данных.** Functor может читать
  ...
  operation.
- **Слой модели становится проще и безопаснее.** Единоличное владение I/O-графом,
  выбор, отделённый от хранения, value-vs-notification отделены, конструирование
  событий, проверяемое в compile-time, валидируемый процесс и оба дефекта устранены
  — фундамент, на котором строится последующая работа по элементам.
...
- **Реализовать полный multiple-set-выбор (§10.4.2 стандарта).** Упорядоченное
  ...
  принципу моделирования (§2.8); структура остаётся открытой к его добавлению, если
  появится реальный спрос.
