...
| **Uncaught BPMN error** — a modeled Error that found no boundary and no scope-chain catcher | Incident at the throwing node. The standard leaves the reaction open (§3); halting recoverably dominates dying, because an uncaught error is routinely a *deployment* mistake — a missing boundary — fixable by model correction and retry against live state |
