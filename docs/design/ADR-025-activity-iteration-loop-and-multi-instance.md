...
own execution frame, which is the per-execution data boundary (ADR-010 v.2) and
therefore the isolation. A composite additionally opens its child scope per
iteration — not for isolation, but because a Sub-Process body *is* a scope and
needs one to exist at all.
...
- **Hydration has an ordering obligation**: rebuild the executors at their
  ...
  before any delivery can be accepted**. A point-to-point message with no
  subscriber is dropped rather than queued, so an out-of-order re-arm loses a
  message silently — the failure this ADR most wants designed out.
