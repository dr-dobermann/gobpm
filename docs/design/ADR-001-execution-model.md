...
|---|---|---|---|
| v.1 | 2026-05-29 | Ruslan Gabitov | Initial Draft. Single-layer Orchestrator + Token model. Superseded — did not reflect the existing two-layer code structure or the token-fork-creates-new-tracks decision. |
| v.2 | 2026-05-29 | Ruslan Gabitov | Full rewrite. Three-layer ownership (Instance + track + token) using existing project vocabulary. Fork creates new tracks registered with Instance. Two event streams (TokenEvent BPMN-observable; TrackEvent execution-observable). Explicit "Conception vs Current Code" section listing the 6 deliberate departures from current implementation. RU twin deferred per bilingual docs policy (translation triggered at Accepted state, not in Draft). |
| v.2 | 2026-05-29 | Ruslan Gabitov | Code-review-round amendments to v.2 Draft (no version bump — pre-acceptance iteration): §3.1 three-layer choice marked tentative (may collapse to two-layer if implementation shows the separation doesn't carry weight; would warrant v.3); §4.1 explicit track:token = 1:1 always, with track constructed as `(startNode, token)`; §4.1 Instance does NOT hold tokens directly — accessed via `track.Token()`; §4.6 fork refactored: parent track survives and continues on F1, N−1 new tracks for F2…FN (not "parent ends, N new tracks"); §4.7 join refactored: first-arrived track survives, all others end via `TrackMerged`, no new track at join; §4.6/§4.7 generalized to "any FlowNode with multiple in/out flows" — Activity-as-fork-point and Activity-as-join-point are first-class, not gateway-exclusive; §4.9 persistence wording fixed — track has no "before it starts" phase separate from its first step; §6 expanded to 10 deliberate departures (added: track:token cardinality, Instance↔token ownership, join mechanics, fork/join-at-activities, track-token link rewrite). |
