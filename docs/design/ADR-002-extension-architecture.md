...
- `ClusterCompatibility` is queried by the runtime layer at startup when cluster mode is active; any `(false, reason)` return is a hard startup failure. (Substantive cluster design lives in the future Distribution & Scale ADR; per [SAD-001 §13.5](SAD-001-vision-and-architecture.md).)
