...
[ADR-040 v.1](ADR-040-process-io-contract.md). The Start/End special case
(process `DataInput`s as targets of a Start event's output associations;
process `DataOutput`s as sources of an End event's input associations) stays
part of the conception and lands with the event attachment capability
([#329](https://github.com/dr-dobermann/gobpm/issues/329)) that it needs.
