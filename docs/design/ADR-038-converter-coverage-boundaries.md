...
issue closes with the landing rather than being re-scoped. The second row
went the same way: the process-level I/O carrier
([#330](https://github.com/dr-dobermann/gobpm/issues/330)) landed as
[ADR-040 v.1](ADR-040-process-io-contract.md)'s declared contract — the
carrier, the launch binding, the completion reading — and the converter's
`<ioSpecification>`-under-`<process>` row followed it; the Start/End
process data path the row also named waits on the event attachment
capability ([#329](https://github.com/dr-dobermann/gobpm/issues/329)),
where it belongs.
