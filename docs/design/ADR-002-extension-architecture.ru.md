...
После того как `New` завершает связывание, Thresher эмитит свой стартовый отчёт через сконфигурированный `Logger`, на уровне INFO, ровно один раз — после применения опций, до того как движок начнёт принимать работу. Отчёт — это два независимых блока, закрываемых разделительной чертой:

1. **Баннер** — словесный знак движка (ASCII), разрешённая `version` и `last commit` (VCS-ревизия и время, которые Go-toolchain запекает в бинарь). Фиксированный идентификационный заголовок, отвечающий на вопрос *какая это сборка?*
2. **Дамп конфигурации** — по строке на каждое разрешённое расширение уровня движка, каждая несёт Go-тип связанной реализации, так что полное связывание расширений читаемо. Отвечает на вопрос *с чем сконфигурирован этот движок?*

Формат (иллюстративный):
...
configuration:
  ...
  logger:                 *slog.Logger
  … по строке на расширение …
────────────────────────────────────────────────────────────
```

Каждый блок печатает по одной INFO-записи на строку (а не одну плотную структурированную запись), так что человек, читающий сырые логи, видит связывание разложенным, а не втиснутым в одну строку. Каждое значение в дампе — это Go-тип связанной реализации.

**Подавление.** Каждый блок независимо отключаем через функциональную опцию:

- `WithoutBanner()` — опустить блок словесного знака / версии / коммита.
- `WithoutStartupConfig()` — опустить дамп конфигурации по расширениям.

Оба по умолчанию **включены**: отчёт виден по умолчанию, согласно политике наблюдаемости проекта «видимо-по-умолчанию» ([память: политика наблюдаемости](../../)); подавление — явный opt-out, никогда не opt-in. Разделительная черта закрывает отчёт только тогда, когда напечатался хотя бы один блок — подавление обоих даёт полностью молчаливый старт (ни черты, ни записей).

**Почему opt-out (пересмотрено относительно v.1).** v.1 сделала стартовый лог обязательным, без опции, оставив заглушение на конфигурацию Logger пользователя. Практика показала две различные помехи, которые рычаг уровня Logger не адресует, не отбрасывая заодно легитимный прикладной INFO: баннер — это декоративный ASCII, который, эмитясь по записи на строку, есть чистый шум в структурированном агрегаторе логов; а дамп конфигурации многословен для встраивающих, конструирующих много короткоживущих движков или уже фиксирующих связывание в другом месте. Поблочный opt-out убирает шум прицельно, сохраняя громкий дефолт.
...
| Стартовое логирование Thresher | Нет стартового логирования | Thresher эмитит стартовый отчёт (баннер + дамп конфигурации по расширениям) INFO-записями на каждый успешный вызов `New` (§4.4.1) | `logStartupConfig` в Thresher. Видим по умолчанию; каждый блок отключаем через `WithoutBanner()` / `WithoutStartupConfig()` (v.2). |
...
| **Стартовый отчёт** | Тест: сконструировать движок с Logger, который захватывает записи. Убедиться: по умолчанию эмитируемые INFO-записи несут блок баннера (version / last commit) и по строке конфигурации на каждое расширение уровня движка (`repository`, `logger`, `tracer`, `metricsRecorder`, `clock`, `messageBroker`, `expressionEngine`, `authorizationProvider`, `workerDispatcher`) со значениями, совпадающими с именами типов связанных реализаций. С `WithoutBanner()` блок баннера отсутствует; с `WithoutStartupConfig()` строки конфигурации отсутствуют; с обоими — старт молчалив. Проверяет §4.4.1. |
...
- `ClusterCompatibility` запрашивается слоем рантайма при старте, когда активен cluster-режим; любой возврат `(false, reason)` — жёсткий отказ старта. (Содержательный дизайн кластера живёт в будущем ADR Distribution & Scale; по [SAD-001 §13.5](SAD-001-vision-and-architecture.md).)
...
|---|---|---|---|
| v.1 | 2026-06-09 | Руслан Габитов | **Принято.** Архитектура расширений приземлена через тот единственный фундаментальный SRD скелета расширений: девять контрактов расширений уровня движка в `pkg/` с упакованными дефолтами, сборка через функциональные опции, разделение публичного `EngineRuntime` / внутреннего `RuntimeEnvironment` и `ExpressionEngine`/`Clock`, связанные в исполнение. Ключевые решения свёрнуты в этот Draft до приёмки (без построчных строк): `EventHub` оставлен internal (не точка расширения); взаимодействие с человеком `TaskDistributor` отложено в собственный ADR; телеметрия с дефолтами по стоимости; принцип ограниченных-in-memory-дефолтов (§4.2); кросс-адаптерная композиция через внедрённый `EngineRuntime` (§3.5 Паттерн C). Набор приёмки только-дефолты из §7 зелёный (`make ci`). Пины ADR-001 подняты v.3 → v.4. RU-twin отложен (batched). |
| v.2 | 2026-06-16 | Руслан Габитов | **§4.4.1 стартовое логирование пересмотрено.** Ослаблена позиция v.1 «обязательно, не опционально»: стартовый отчёт теперь два независимо отключаемых блока — `WithoutBanner()` и `WithoutStartupConfig()` — по-прежнему видимых по умолчанию (opt-out, никогда не opt-in). Прозу §4.4.1 приведено в соответствие с тем, что отгружено (баннер + по одной INFO-записи на расширение, вместо изначально иллюстрировавшейся одной структурированной строки `thresher.starting`); разделительная черта закрывает отчёт только если блок напечатался, так что подавление обоих полностью молчаливо. Строка решения §6 и строка приёмки §7 обновлены. Приземлено сопровождающим реализующим SRD. |
