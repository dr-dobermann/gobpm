...
| Escalation | ❌ отложено | — |
| Cancel | ✅ реализовано позже | un-defer через [ADR-028](ADR-028-transaction-sub-process.md) — прерывание Transaction Sub-Process (§10.7): всегда прерывающая, **декларированный в модели выход**, разрешаемый loop-локально в точке прерывания, а не hub-вооружённая граница (аналог класса прямого разрешения Error/Escalation) |
