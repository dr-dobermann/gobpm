...
| `<ioSpecification>` on a `<process>` | the model has no process-level I/O carrier (ADR-011 §2.5's planned work); a task's `<ioSpecification>` imports | declare the data on the tasks that read or write it | [#330](https://github.com/dr-dobermann/gobpm/issues/330) |
| a data association, or a bare `<dataInput>`/`<dataOutput>`/`<inputSet>`/`<outputSet>`, on an event | the model's events have no attachment for a data association | move the data through a task beside the event | [#329](https://github.com/dr-dobermann/gobpm/issues/329) |
