...
| Containers — `subProcess` (incl. event sub-process), `transaction`, `callActivity` | `activities.NewSubProcess` / `NewCallActivity`. A `calledElement` may be qualified by another document's namespace, which the host's `thresher.WithCallableResolver` maps onto a registry key |
| The `GlobalTask` family — `globalTask` and its four typed siblings | one `process.Process` each: a None start, the task built as its in-process counterpart, a None end, registered under the global task's id |
