...
| Выявлено из | [SRD-001](../srd/SRD-001-multi-module-scaffold.md) — `-race` теперь гейтит CI; существовавшая гонка становится видимой |
...
`-race` теперь гейтит CI согласно [SRD-001 §3 / §5](../srd/SRD-001-multi-module-scaffold.md). Без устранения этой гонки CI на master падает периодически.
...
- SRD-001 добавил `make ci` с race-гейтом для `test-all`, плюс правила depguard. Запуск `make ci` локально и на CI надёжнее провоцирует тайминг, при котором гонка срабатывает.

Хрупкость была всегда; SRD-001 просто перестал давать ей проскальзывать.
...
- [SRD-001 v.1 §5 Tests / Verification](../srd/SRD-001-multi-module-scaffold.md) — `-race` — это CI-гейт, который выявляет эту и подобные ошибки.
