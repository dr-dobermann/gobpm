...
**Решено (2026-06-29, SRD-031.B).** `state` переведён на `atomic.Uint32` —
`State`/`UpdateState`/`ensureStarted` теперь lock-free, поэтому вектор
RC2-self-deadlock (чтение состояния под `t.m`) устранён по построению, а не
соглашением. Все критические секции под `t.m` вынесены в `...Locked`-хелперы
(`pkg/thresher/locked.go`), возвращающие плоские данные и не вызывающие
EventHub/launchInstance, — лок физически не удерживается через вызов
подсистемы. `Run`/`Shutdown` переведены на compare-and-swap через переходные
состояния `Starting`/`Stopping` (детерминированный отказ при двойном
запуске/останове).
