...
| Implements | [ADR-007 v.2.1](../design/ADR-007-in-memory-long-waits.md) (the whole mechanism) + [ADR-033 v.2](../design/ADR-033-persistence-and-state.md) §2.4/§2.5 (the durable projection, the wait-holders) |
...
- **§4.2 Lease-while-dehydrated (single-engine scope).** A dehydrated
  ...
  coordination — deferred to the Distribution & Scale ADR (ADR-007 §5). The dehydrated
...
- Implements **ADR-007 v.2.1** (whole) + **ADR-033 v.2** §2.4/§2.5.
...
- **Single-engine scope.** A second engine claiming a lapsed-lease dehydrated
  instance remains the Distribution & Scale ADR's territory (§4.2).
...
| v.2.5 | 2026-07-29 | Ruslan Gabitov | Pins refreshed to **ADR-007 v.2.1**, which now carries the boundary concept this SRD implements — FR-9a realizes §2.4's per-wait eligibility and §2.3's trigger-absent boundary wake, neither of which existed in the ADR at v.2. The implementation established the concept; the ADR is where it belongs, and the pin follows it. |
