...
| FR-7 | **Document** the event-subscription keying boundary: the EventHub keys waiters by `eDef.ID()` **alone** (`eventhub.go:118/145`), and `Event.clone()` shares event definitions by reference (M1), so two instances' cloned event-start/catch nodes carry the same definition id and would share a waiter. Per-instance **event-subscription** isolation (keying by `(processor, defID)` / per-instance subscriptions) is **event-delivery semantics owned by ADR-006**, not the per-instance node-state decision of this SRD — **deferred to ADR-006**. The per-instance node-state race this SRD eliminates lives on **plain** flow (no event waiters); FR-1…6 and the V4 race test cover it on a plain process. |
...
- **M2 — wire `Snapshot.Clone()` + `instance.New`.** `snapshot.(*Snapshot).Clone()` mirroring `snapshot.New`'s node-loop/flow-loop (clone nodes via `Clone()`, relink flows via the edge helper, **remap each gateway's `defaultFlow` to the cloned edge**, share immutable header); `instance.New` swaps the shared snapshot for its clone (FR-1); **document** the event-subscription keying boundary deferred to ADR-006 (FR-7); **`-race` independence test** — two concurrent instances of one **plain** process (Start→ServiceTask→End), no race + independent `dataPath`/operation/`scope`; all examples run.
