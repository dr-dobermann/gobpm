...
|---|---|---|
| `TestParallelMultiInstanceRunsAll` | instance | FR-1/FR-2 N instances all run (bodies fire N times) |
| `TestParallelMultiInstanceDistinctScopes` | instance | FR-2 N distinct `-i` segments, concurrent |
| `TestParallelMultiInstanceZeroCardinality` | instance | FR-2 N ≤ 0 → zero instances, completes |
| `TestParallelMultiInstanceCompletesWhenLastDrains` | instance | FR-3 host resumes only after the N-th drain |
| `TestParallelMultiInstanceInputItemPerScope` | instance | FR-5 each instance sees its own element |
| `TestParallelMultiInstanceAssemblesOutput` | instance | FR-6/FR-7 positional output, published once |
| `TestParallelMultiInstanceCompletionCancelsRemainder` | instance | FR-8 `true` cancels still-open instances, `numberOfTerminatedInstances` |
| `TestParallelMultiInstanceRuntimeAttributes` | instance | FR-9 `numberOfActiveInstances` > 1, decrementing; terminated count |
| `TestParallelMultiInstanceBoundaryInterruptsAll` | instance | FR-10 interrupting boundary tears down all N |
| `TestParallelMultiInstanceEmitsIterationFacts` | instance | FR-11 per-instance scope facts carry the ordinal |
| `TestParallelMultiInstanceSequentialUntouched` | instance | NFR-2 sequential MI + Standard Loop still pass (suite) |
