...
| **FR-3a** | `Claim(taskID, actor)` succeeds iff the actor is eligible **and** the task is not held by a **different** actor; over another's hold it fails with a distinct, non-terminal "already held" error. Re-claiming a task the actor already owns is an **idempotent no-op** *(corrected in v.1.4)* — the guard protects against taking another's work, and refusing a same-owner claim would make the operation unsafe to retry and leave a born-owned task (FR-6) uncompletable by its own assignee. |
...
| v.1.7 | 2026-07-30 | **M5, M6 and M7 landed.** M5 proves the cancellation rule ownership most invites doubt about: a **held** task is torn down exactly like an unheld one — the end-to-end cancel test now claims before cancelling, and afterwards the holder can neither complete, re-claim, release nor hand it on. M6 adds `docs/bpmn-spec/semantics/instance-attributes.md` **by hand**, because `elements/` is generated from bpmn-moddle's XML metamodel and therefore *structurally cannot* emit instance attributes — the index and `semantics/tasks.md` now say so explicitly, so the next reader does not repeat ADR-020 v.1's inference that the standard is silent on ownership. M7 was mostly already delivered by M2 (the console driver claims, `examples/dehydration` claims); what remained was narration — the `usertask` banner and the examples README described a Take→Complete flow that no longer exists. Also added a direct unit test for `RuntimeVar(COMPLETED_BY)`: the end-to-end test could not cover the **empty** register, because by the time it reads one a record always exists. |
| v.1.8 | 2026-07-30 | **M8 landed — the ADR-020 Russian twin is at v.2.3**, 519 → 958 lines against the English's 945. Translated section-for-section rather than regenerated: §1.4, §2.1.1, §2.4.1, §2.4.2, §2.5.1–§2.5.3, the §2.7 contract change, §3's instance-attribute and prior-art rows with the pin-provenance note, §4's seven ownership alternatives, §5–§7's additions, §8's references, and four history rows. The twin also carries the same three corrections the English received: `ADR-006 v.2`→`v.4`, the removal of downward `SRD-027/028` and `SRD-034` citations that the hierarchy rule forbids, and the stale "dehydration is deferred" statements that ADR-007 v.2.1 superseded — a twin that kept them would have been a faithful translation of a wrong document. All milestones are now landed; what remains is the `/check-srd` audit, §10, the status flips and the PR. |
