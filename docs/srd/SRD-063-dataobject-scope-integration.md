...
- **FR-5 — DataObject data flow routes through scope (ADR-030 §2.3).** A task's DataAssociation to a DataObject writes the task's output to the **scope entry named by the DataObject** (through the execution frame — the plane every output already uses), not into the DataObject's item-aware element. Every reader resolves it **by name** via the walk-up. This **retires** the object side-channel (`task.UploadData` writing into the target IAE) and the unused `DataObject.Update()`. Per-instance isolation is **free** — the scope is per-instance, so the seeded clone needs **no** association-retargeting. `examples/process-data` keeps its process definition but updates its **read** from the model object to the instance's data by name.
