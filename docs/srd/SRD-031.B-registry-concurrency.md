...
Landed on `feat/srd-031b-registry-concurrency` over master `2242bff`.

**Milestones (commits):**
- **B1** `d716b92` — atomic `state` (`atomic.Uint32`) + the `Starting`/`Stopping`
  transitional enum values; `State`/`UpdateState`/`ensureStarted` lock-free;
  `Run` via `CAS(NotStarted→Starting)`→hub→`Store(Started)` with rollback on hub
  failure; `Shutdown` via `CAS(Started→Stopping)`→teardown→`Store(Stopped)`
  (idempotent; `NotStarted→Stopped`; `Paused` torn down; `Starting`/`Invalid`
  rejected); struct lock-contract comment. `New` stores `NotStarted` explicitly.
- **B2** `6ee57bb` — the nine `…Locked` helpers in `pkg/thresher/locked.go`
  (`appendVersionLocked`, `removeVersionLocked`, `removeKeyLocked`,
  `latestStartersLocked`, `reserveKeyLocked`/`releaseKeyLocked`,
  `latestSnapshotLocked`, `snapshotForVersionLocked`, `trackInstanceLocked`);
  `RegisterProcess`/`UnregisterVersion`/`UnregisterProcess`/`registerAllStarters`/
  `resolveAndLaunch`/`StartLatest`/`StartVersion`/`launchInstance*` reshaped onto
  them (the `started` reads moved out, lock-free). Behavior unchanged.
- **B3** `fb8a622` — `TestRegistryConcurrentStress` (T-5); this doc close-out +
  audit §2.6 retirement.

**Files:** `pkg/thresher/thresher.go` (state field/enum, `New`, `State`,
`UpdateState`, `Run`, `Shutdown`, the reshaped registry methods, lock-contract
comment), `pkg/thresher/locked.go` (new), tests
`lifecycle_test.go`/`lifecycle_internal_test.go`/`locked_internal_test.go`/
`registry_stress_test.go` (new) and `thresher_state_test.go`/
`instance_starter_internal_test.go` (adjusted).

**Verification:** T-1…T-10 present and green; `go test -race ./...` green;
`make ci` green incl. **diff-coverage 100%** of 176 changed lines (min 95%) and
`govulncheck` clean; all **18 examples run** end-to-end (exit 0). Touched-function
coverage: `State`/`UpdateState`/`Run`/`ensureStarted`/`UnregisterVersion`/
`UnregisterProcess`/`registerAllStarters`/`resolveAndLaunch`/`StartLatest`/
`StartVersion` and all nine `…Locked` helpers at 100%; `Shutdown` 95.2% (the
concurrent-loser CAS branch is exercised by T-5 in the full run).

Audit §2.6 (MAJOR) retired by this SRD.
