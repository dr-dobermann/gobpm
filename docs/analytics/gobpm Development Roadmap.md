...
- Accept **SAD-001**: requires §13 Distribution & Scale to be refined or relocated to a dedicated **Distribution & Scale ADR** first (it is explicitly flagged preliminary).
...
Deferred until multi-node demand materialises; gated on WS-B3 persistence. Specified in a future **Distribution & Scale ADR** (the home for SAD §13's preliminary content):
