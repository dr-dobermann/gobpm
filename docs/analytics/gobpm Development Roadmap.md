...
- **Statuses:** SAD-001 v.1 Draft; **ADR-001 v.5 Accepted**; **ADR-002 v.1 Accepted**; ADR-003 / ADR-004 v.1 Draft; **ADR-005 v.1 Accepted** (gateways & joins — Parallel authored in full, landed via SRD-005); **ADR-006 / ADR-007 v.1 Draft** (events/long-waits, relocated from ADR-001); **ADR-009 v.1 Accepted** (per-instance node graph — node-owned runtime state; decides the ADR-001 §4.7 deferral and eliminates the shared-node data race); **ADR-010 v.1 Accepted** (process data model — container-scope data plane + per-execution frames); **SRD-001 v.1 Accepted** (instance/track/token refactor); **SRD-005 v.1 Accepted** (Parallel gateway split + synchronizing join); **SRD-006 v.1 Accepted** (per-instance cloning, lands ADR-009); **SRD-007 v.1 Accepted** (lands ADR-010); **SRD-008 v.1 Accepted** (lands ADR-011's model-layer hardening — single-ownership I/O graph, GetKeys/RemoveParameter defects, Process.Validate at registration); **ADR-011 v.1 Accepted** (process data flow — model-layer semantics: one input/output set per activity, availability-gated start with no data wait, the three association shapes, a narrow public service data reader, model-layer hardening; first part landed via SRD-008, remaining via its implementing SRDs — single-set evaluation → service reader); FIX-001 v.1 **Accepted**; **FIX-003 v.1 landed** (audit event-subsystem + track-state bug sweep: timer close-owner, unregistration chain, RegisterEvent TOCTOU); **SRD-002 / SRD-003 / SRD-004 Accepted** (CI gates, covercheck, extension skeleton). (ADR-008 Distribution & Scale — planned, the home for SAD §13. ADR-012 layering / ADR-013 instance observability — queued per the 2026-06-11 architecture-audit triage. The two deliberate BPMN deviations ADR-011 decides — no data-availability wait, no multiple I/O sets — are registered in SAD-001 v.1 §14.1.)
