...
func (t *track) runLeafMISequential(
	...
	for i := start; i < n; i++ {
		...
		// bind loopCounter, the §2.9 counts and inputItem=collection[i]
		// at the host scope — the activity's frame resolves them by
		// walk-up, exactly as a seeded composite body does.
		...
		// the behavior event carries the CURRENT §2.9 counts
		// (SRD-056.B): republish post-drain, then throw.
