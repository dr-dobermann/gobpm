...
// Plane is the per-instance data plane: the container-scope tree and the
...
// (SRD-007 M4), Plane is renamed to Scope.
type Plane struct {
...
// NewPlane creates a data plane rooted at root with the root container scope
...
func NewPlane(root DataPath, rt RuntimeVarsSupplier) (*Plane, error) {
	if err := root.Validate(); err != nil {
		return nil,
			errs.New(
				errs.M("NewPlane: invalid root path %q", root),
...
func (p *Plane) GetData(from DataPath, name string) (data.Data, error) {
...
func (p *Plane) GetDataByID(from DataPath, id string) (data.Data, error) {
...
func (p *Plane) Commit(at DataPath, dd ...data.Data) error {
...
func (p *Plane) OpenScope(path DataPath) error {
...
// closed first.
func (p *Plane) CloseScope(path DataPath) error {
...
func (p *Plane) checkContained(op string, path DataPath) error {
...
func (p *Plane) checkWritable(op string, path DataPath) error {
	if path == p.rtPath ||
