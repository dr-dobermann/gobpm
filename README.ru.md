...
- [Конвертеры обмена](docs/guides/extending/converters.md) — импорт и экспорт BPMN 2.0 XML: формат-агностичный шов `convert`, регистрация через blank-import, сохранение `id` как ключа версии, обратная связь по неподдерживаемым элементам, семантический round-trip, переиспользование по ссылке (`<globalTask>` импортируется как вызываемый процесс; `calledElement` с пространством имён отображает резолвер хоста `WithCallableResolver`) (EN)
