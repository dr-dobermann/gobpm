...
func echoMessage(ev Fact) string {
	if ev.Phase == "" {
...
// echoReporter is the default Reporter (ADR-013 v.2 §2.7): echo-only. It writes each
...
type echoReporter struct {
	log Logger
}

// NewEchoReporter returns the default echo-only sink bound to log. A nil log falls
// back to slog.Default(), so the sink is never silent.
func NewEchoReporter(log Logger) Reporter {
	if log == nil {
		log = slog.Default()
	}

	return &echoReporter{log: log}
}

// Report echoes a loggable event to the operator log (the Reporter contract).
func (s *echoReporter) Report(ev Fact) {
	Echo(s.log, ev)
}
